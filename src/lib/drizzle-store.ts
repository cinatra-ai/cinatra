...
END $$`,
  ...
  WHERE key = 'source_config:content-blog'`,
    ...
    { text: `ALTER TABLE "${schemaName.replaceAll('"', '""')}"."agent_runs" ADD COLUMN IF NOT EXISTS human_present boolean` }, { text: `ALTER TABLE "${schemaName.replaceAll('"', '""')}"."agent_runs" ADD COLUMN IF NOT EXISTS delegated_actor_snapshot text` }, ...agentRunAssignmentScopeSchemaQueries(schemaName), { text: `ALTER TABLE "${schemaName.replaceAll('"', '""')}"."agent_runs" ADD COLUMN IF NOT EXISTS launch_scope_anchor jsonb` }, // launch_scope_anchor (cinatra#2809, epic #2806): the IMMUTABLE vantage the run was LAUNCHED from, which decides its ONE canonical address; absent / malformed resolves to unanchored and the flat bare route, with no backfill and no inference from another column. Its statement is written INLINE here rather than in a shared builder beside the rule: a new leaf module would enter four LOCKED route graphs (every route reaches this DDL owner) whose module counts may only ever shrink, so the statement rides this existing line — THIS module is at its file-size ceiling too — and the parity suite pins it against its assistant_threads twin and the migration. Operator-upgrade twin = migrations/core/core__0101. // delegated_actor_snapshot: the requesting user's ActorContext, captured at instantiate and replayed at run-start re-authz + mid-run authz checks (nullable JSON text; legacy rows fall back to live-session derivation). assignment_scope_snapshot (cinatra#2813 S1, epic #2812): the IMMUTABLE scopes a run was created under, decided ONCE at creation because the scope a run's assignments come from must not be able to move under it; absent / malformed / unknown-version resolves to workspace plus the durable org and nothing else. Its statement moved into src/lib/assignment-scope.ts — the leaf that already owns this slice's SQL and that this composition already reaches — because THIS module is at its file-size ceiling, which may only ever shrink; the spread rides this line for the same reason as the import, and the leaf carries the reasoning in full. Operator-upgrade twin = migrations/core/core__0100.
