...
END $$`,
  ...
  WHERE key = 'source_config:content-blog'`,
    ...
    ...lifecycleRepairSchemaQueries(schemaName), ...suggestionDecisionCasSchemaQueries(schemaName), ...agentRunHitlGatesSchemaQueries(schemaName), ...runRecommendationSkipsSchemaQueries(schemaName), // Both AFTER the S0 spread, which creates the tables they alter. agent_run_hitl_gates (cinatra#2748): the DURABLE human-approval gate artifact, so a Redis event-log expiry can no longer destroy a paused run's only answerable gate; AFTER agent_runs above, which its FK references; migration twin core__0093. S2 (cinatra#2040): repair loop (durable batch epoch, per-epoch aggregate, repair lineage) + changes_requested CHECK-widen + rejected-efficacy row; migration twin core__0081. chat-hitl S6b (cinatra#2571): the per-item suggestion-decision ledger reshape + the application-intent outbox; migration twin core__0092. run_recommendation_skips (cinatra#2794 S9b): the RUN-LEVEL skip record that replaces the reserved-skill-id marker, keyed by run_id alone; migration twin core__0094. Spread on ONE line deliberately — this module sits at its file-size ceiling, which may only ever shrink.
