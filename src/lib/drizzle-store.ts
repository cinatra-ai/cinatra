...
import { agentAssignedContextSchemaQueries } from "@/lib/agent-assigned-context-schema";
...
END $$`,
  ...
  WHERE key = 'source_config:content-blog'`,
    ...
    // scopes a run was created under — {v, orgId, projectId?, teamIds[],
    // originatingHumanUserId?}. Decided once, at creation, because the scope a
    // run's assignments come from must not be able to move under it. Absent /
    // malformed / unknown-version resolves to workspace + the durable org and
    // nothing else. Operator-upgrade twin = migrations/core/core__0100.
    { text: `ALTER TABLE "${schemaName.replaceAll('"', '""')}"."agent_runs" ADD COLUMN IF NOT EXISTS assignment_scope_snapshot jsonb` },
...
EXCEPTION WHEN duplicate_object THEN NULL; END $$`,
    ...
    ...agentAssignedContextSchemaQueries(schemaName), // agent_assigned_context (cinatra#2813 S1): the ARTIFACT twin of agent_assigned_skills, same exact-scope tuple rule, artifact FK ON DELETE CASCADE. Fresh-install half; operator-upgrade twin = migrations/core/core__0100. Spread AFTER the resource table its FK references.
