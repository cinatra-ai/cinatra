...
import { artifactReviewGateSchemaQueries, lifecycleInterceptionsSchemaQueries, lifecycleRepairSchemaQueries, suggestionDecisionCasSchemaQueries } from "@/lib/artifacts/artifact-review-gate-schema";
...
END $$` },
  ...
  WHERE deleted_at IS NULL` },
    ...
    ...chatCaptureSchemaQueries(schemaName), // chat-capture turn ledger (cinatra#1367), additive — mirrors core__0042
...
END $$`,
  ...
  WHERE key = 'source_config:content-blog'`,
    ...
    ...lifecycleRepairSchemaQueries(schemaName), ...suggestionDecisionCasSchemaQueries(schemaName), // Both AFTER the S0 spread, which creates the tables they alter. S2 (cinatra#2040): repair loop (durable batch epoch, per-epoch aggregate, repair lineage) + changes_requested CHECK-widen + rejected-efficacy row; migration twin core__0081. chat-hitl S6b (cinatra#2571): the per-item suggestion-decision ledger reshape + the application-intent outbox; migration twin core__0092. Spread on ONE line deliberately — this module sits at its file-size ceiling, which may only ever shrink.
...
$body$` },
    ...
    // Extracted to the pure-strings leaf semantic-assertion-schema.ts
    // (cinatra#1426; the artifact-claim-schema.ts extract-leaf pattern):
...
END $$` },
    ...
    { text: `CREATE INDEX IF NOT EXISTS anthropic_skill_lease_expires_idx ON "${schemaName.replaceAll('"', '""')}"."anthropic_skill_lease" (expires_at)` }, ...skillBundleSchemaQueries(schemaName), ...skillUploadConsentSchemaQueries(schemaName), // cinatra#2088 bundle authority + cinatra#2092 S5 consent ledger / reconcile outbox — spread HERE: needs skill_revisions (above) AND anthropic_skill_sync (just created)
