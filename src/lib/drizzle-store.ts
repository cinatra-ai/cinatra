...
END $$` },
    ...
    //   - The locked plan called for NOT NULL, but the existing
    //     `notifications` table predates this phase as a generic
    //     (id, payload) JSON-row store. Pre-existing rows have no user_id
    //     and a destructive backfill is not in scope for B1.
...
END $$` },
    ...
    // (NOT re-dispatch from start). ADR 0043.
    ...
    // Gap G5 — timeout_seconds: server-side run timeout.
...
$body$` },
    ...
    //    matcher draft can never become eligible by UPDATE either; FM-6);
