...
END $$` },
    ...
    // token_hash is the PRIMARY KEY so the verifier stays ONE index probe (no newest-wins, no body fallback); new table ⇒ additive; growth bounded by a per-run cap on insert. See packages/agents/src/run-token-store.ts.
