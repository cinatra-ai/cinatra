 ...
 * exactly the same shim shape as `applyPasswordToggleA11y` above: idempotent,
 * keyed off the library's own stable `data-slot` attributes (`form-item`,
 * `form-control`), never touches `document` globally, and is safe to re-run
 * from a MutationObserver. It is co-located here (rather than in its own
 * module) because it is the same better-auth-ui auth-form DOM shim, scoped to
 * the same rendered sign-in subtree.
 *
