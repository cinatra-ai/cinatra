...
export const RAW_OBJECT_ACCESS_ALLOWLIST: readonly RawObjectAccessEntry[] = [
  ...
  { file: "src/lib/artifacts/artifact-revision-append.ts",    category: "substrate", note: "artifacts substrate (cinatra#3080, fix leg 8) \u2014 the APPEND-A-REVISION writer beside artifact-creation.ts: it files the next append-only representation row of an artifact that already exists (the change road's Regenerate settles its successor over the SAME artifact) and, in that same transaction, moves that artifact's own envelope pointer onto the new revision. It names the objects table for two things only, both mirroring artifact-creation.ts: the guarded liveness read of the target row (deleted_at IS NULL, re-asserted inside the lock), and the `data = data || patch` pointer update on that one row. It writes no identity \u2014 no row is created, no type is asserted, no scope is changed \u2014 and it holds no package-name branch: the type is read off the row it appends to" },
