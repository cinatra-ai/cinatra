...
export const RAW_OBJECT_ACCESS_ALLOWLIST: readonly RawObjectAccessEntry[] = [
  ...
  { file: "src/lib/artifacts/typed-promotion.ts", category: "substrate", note: "artifacts substrate (cinatra#3028, epic #3023 — enabler 0.14, the typed promotion road) — a PURE query BUILDER with no connection and no execution: it returns the compare-and-set text that retypes a matched base-typed row into the matching extension's own type (anchored on objects.version AND the current type) plus the guarded representation append that shares the base revision's resource. It matches the raw-table pattern because it NAMES the objects table in that SQL, not because it accesses rows" },
  { file: "src/lib/artifacts/typed-promotion-store.ts", category: "substrate", note: "artifacts substrate (cinatra#3028, epic #3023 — enabler 0.14) — the road's store half: READS the base-typed row with its latest representation and the blob's own detected mime (the promotion re-validates the shared content against the target type's accepted forms), and runs the pure builder's compare-and-set transaction. The only objects WRITE is that retype, which is a type change on the row the person confirmed and is anchored so a lost race writes nothing" },
