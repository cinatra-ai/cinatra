...
export const RAW_OBJECT_ACCESS_ALLOWLIST: readonly RawObjectAccessEntry[] = [
  ...
  { file: "src/lib/artifacts/run-artifact-records.ts", category: "substrate", note: "artifacts substrate (cinatra#3029, epic #3023 — W5, the default road and the detection ladder) — READ-ONLY against the objects table: the run page reads the produced and the consumed artifact records of one run, and each of the two statements JOINs objects on the artifact id purely to carry the artifact identity forward (its type and data) alongside the representation revision mime. It never writes objects, and it holds no predicate over object rows beyond that join" },
