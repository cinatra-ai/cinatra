...
export const RAW_OBJECT_ACCESS_ALLOWLIST: readonly RawObjectAccessEntry[] = [
  ...
  { file: "src/lib/artifacts/artifact-revision-append.ts", category: "substrate", note: "artifacts substrate (cinatra#3030, epic #3023 W6 — the mid-run same-artifact revision) — the APPEND side of artifact-creation.ts above and the same CTE-atomic-writer class: one atomic append transaction whose compare-and-set is the representation table's own unique index on (org_id, artifact_id, revision), carrying its own artifact_audit writer witness and its conditional artifact_produced_outbox row. It touches the objects table twice: it READS the artifact's declared type before any byte is written, so a refusal costs nothing, and its only objects-table DML is the projection mirror — an UPDATE that moves the three cached pointer fields artifact-creation.ts writes when it creates the row (latestRepresentationRevisionId, latestDigest, mime) and rewrites nothing else. It emits NO canonical object-history event and NO objects.version bump; like the artifact stores above, migration to the canonical writer is a follow-up. The objects-writer drift gate admits this file in the same class for the same reason." },
