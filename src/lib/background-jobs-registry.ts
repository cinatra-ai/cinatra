...
// ---------------------------------------------------------------------------
// Enqueue-time reuse of a jobId whose previous job has SETTLED.
//
// Lives HERE, beside the dispatcher, rather than in a module of its own: this
// registry is already on the reachable graph of every locked dev-perf route
// through `background-jobs.ts`, so hosting the helper adds NO module and NO
// import edge (the BullMQ `Queue` arrives TYPE-ONLY, erased at compile). A
// separate leaf file would have added one first-party module to each of those
// routes for a 20-line function — the route-graph ratchet's whole subject.
//
