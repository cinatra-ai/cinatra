...
export const PRIMITIVE_CLASSIFICATIONS: Record<string, PrimitiveClassification> = {
  ...
  // enforcement delta at the coarse Posture-B boundary. The REAL authorization
  // is the module's own input-time authorizeAssistantMcpTurn (handle registry
  // resolution + mcp.enabled/restriction target policy + the generalized G2
  // thread/grant check with sealed-room 404-hide) — all identity from
  // mcpRequestContextStorage, never tool input.
