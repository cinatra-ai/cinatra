...
// ===========================================================================
// Shared objects + graphiti_projection_outbox single-CTE builder
// (cinatra#1894, epic #1883 §D7 / decomposition #1944 B1b — drift-control tier b).
//
// Relocated here (cinatra#1894 B1b) from the former standalone
// `src/lib/objects/objects-outbox-cte.ts` so it co-locates with the object-write
// query builders it belongs beside (`buildSoftDeleteObjectQuery`,
// `upsertObjectAndEnqueue`) AND stops being a NET-NEW first-party module on the
// artifact-creation hot path's route graph (route-graph ratchet, cinatra#732):
// `objects-store.ts` is already reachable from every tracked route, so
// artifact-creation reaching the builder through it adds ZERO graph pressure.
// The builder itself is unchanged (byte-identical output — pinned by the golden
// test) and remains the ONE shared shape for both consumers.
//
...
//
// A parity/contract test (`objects-outbox-cte.test.ts`) pins
// `OBJECTS_WRITE_COLUMNS` against the live objects-write column set so a future
// canonical column-add the twin misses fails CI (drift-control tier c).
// ===========================================================================
...
function objectsOutboxSourceLiteral(source: string): string {
...
export function buildObjectsWithOutboxQuery(
  ...
  const src = objectsOutboxSourceLiteral(input.source);
