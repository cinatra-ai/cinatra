...
/**
 * Capture (or honestly degrade) the pinned preview for one gate target.
...
export async function capturePinnedPreview(
  ...
  const degrade = async (
    ...
    extra?: { sourceOrigin?: string | null; postId?: number | null },
  ): Promise<CapturePinnedPreviewOutcome> => {
    try {
  ...
  try {
    ...
    if (!target.ok) return await degrade(target.reason);

    // 2. CREDENTIAL — the connect-provisioned shared secret, via the existing
    // host signer. A fresh `webhook-id` per attempt (the plugin consumes it
    // single-use), so a legitimate retry is never a replay.
    const secrets = await deps.resolvePreviewSecrets(target.siteId);
    if (secrets.length === 0) {
      return await degrade("no-preview-credential", {
    ...
    if (!fetched || !fetched.ok) {
      return await degrade(fetched?.reason ?? "preview-unreachable", {
        sourceOrigin: target.origin,
        postId: target.postId,
      });
    }
    ...
    if (violations.length > 0) {
      ...
      return await degrade("sanitization-failed", {
        sourceOrigin: target.origin,
        postId: target.postId,
      });
    }
    ...
    const rendered = await deps.renderIsolated({
      ...
      // string would leave a DNS-rebind hole the parent's guard already closed
      // for its own request (a codex convergence finding): the name could
      // re-resolve to an internal address between the signed fetch and the
      // subresource load. So the guard-validated addresses travel with the
      // page, and the renderer pins its resolver to exactly them.
      pinnedAddresses: fetched.pinnedAddresses,
      timeoutMs: RENDER_TIMEOUT_MS,
    });
    if (!rendered.ok) {
      return await degrade(rendered.reason, {
        sourceOrigin: target.origin,
        postId: target.postId,
      });
    }
    ...
    const capture = await deps.writeCapture({
      ...
      data: {
        ...
        captureDigest: createHash("sha256").update(sanitized.html).digest("hex"),
        title,
      },
    });
    ...
    return await degrade("render-failed");
  }
}
...
/**
 * The entry point the CMS-review host seam calls at gate creation. Bounded by a
 * hard wall-clock ceiling and swallowing every failure: the staged write and its
 * gate proceed regardless of what the capture does.
 */
export async function capturePinnedPreviewForGate(
  input: CapturePinnedPreviewInput,
): Promise<CapturePinnedPreviewOutcome> {
  const deps = createPreviewCaptureDeps();
  let timer: NodeJS.Timeout | undefined;
  const ceiling = new Promise<CapturePinnedPreviewOutcome>((resolve) => {
    timer = setTimeout(
      () => resolve({ status: "degraded", reason: "capture-timeout", capture: null }),
      CAPTURE_TOTAL_TIMEOUT_MS,
    );
  });
  try {
    return await Promise.race([capturePinnedPreview(input, deps), ceiling]);
  } finally {
