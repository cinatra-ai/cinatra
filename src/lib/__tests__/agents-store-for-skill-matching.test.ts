...
describe("readAgentsForSkillMatching", () => {
  ...
  it("MATCH-AGENTS-01: includes installed templates with packageName", async () => {
  ...
  it("MATCH-AGENTS-02: drops templates with null/empty packageName", async () => {
    readInstalledAgentTemplatesMock.mockResolvedValue([
      {
        ...baseTemplate,
        id: "tpl-with-pkg",
  ...
  it("MATCH-AGENTS-03: maps to PersistedAgent shape (packageId + name + description + keywords + slug-derived id)", async () => {
    readInstalledAgentTemplatesMock.mockResolvedValue([
      {
        ...baseTemplate,
        id: "tpl-x",
  ...
  it("MATCH-AGENTS-04: returns [] when the upstream reader throws (defensive)", async () => {
