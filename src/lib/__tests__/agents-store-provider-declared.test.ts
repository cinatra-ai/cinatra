...
describe("readProviderDeclaredAgents + readAgentsForSkillMatching union", () => {
  ...
  it("PROVIDER-01: surfaces a new-canonical (oas.json) provider agent", () => {
  ...
  it("PROVIDER-02: walks transitional and legacy layouts (4-rung resolver parity with handleAgentBuilderGitList)", () => {
  ...
  it("PROVIDER-03: falls back to sibling package.json for packageName + description", () => {
  ...
  it("PROVIDER-04: drops agents with no resolvable packageName (cannot be keyed in skill_matches)", () => {
  ...
  it("UNION-01: union of DB-installed + provider-declared agents", async () => {
  ...
  it("UNION-02: DB row wins when packageId collides", async () => {
    readInstalledAgentTemplatesMock.mockResolvedValue([
      {
        ...baseTemplate,
        id: "tpl-shared",
  ...
  it("UNION-03: throwOnError propagates filesystem readdir failures", () => {
  ...
  it("UNION-05: throwOnError propagates a malformed sibling package.json when it's the only packageName source", () => {
  ...
  it("UNION-04: throwOnError propagates a malformed per-agent oas.json (so the matcher write path fails closed)", () => {
...
describe("skill matching source contracts", () => {
  it("BUG-2: page.tsx dropdown filters out level=agent skills", () => {
  ...
  it("BUG-3: matchAgentsToSkills does not push self-owned agent skills into the projection", () => {
    const source = readFileSync(
