...
describe("the run a run-bound table's rows belong to (cinatra#3249)", () => {
  ...
  it("fills the run column from the BOUND run on insertIfAbsent, unnamed by the caller", () => {
    ...
    // The conflict lookup stays inside the org, and the run decides the ORDER
    // rather than the membership: this run's own row is preferred, a sibling
    // run's row of the same scope is still found when this run has none.
    expect(c.text).toContain('WHERE "org_id" = $5 AND "id" = $6 ORDER BY ("run_id" = $7) DESC');
    expect(c.values).toEqual(["org-w7", RUN, "r1", "reserved", "org-w7", "r1", RUN]);
  ...
  /**
   * THE RUN COLUMN IS NOT PROJECTED (adopted from the convergence round). The
   * organisation and the scope may be projected — every row a call can reach
   * carries the one organisation and the one scope the call already stands in —
   * but the run is no floor, so a scope-wide read reaches SIBLING runs' rows,
   * and projecting the column would hand the module their run identifiers.
   */
  it("keeps the run column out of the default projection", () => {
    const c = buildRunBound({ operation: "select", table: "run_reservations" });
    expect(c.text).toContain('SELECT "id", "org_id", "state" FROM');
    expect(c.text).not.toContain('"run_id"');
  });

  it("refuses a request that names the run column in its PROJECTION", () => {
    const e = refusal(() =>
      buildRunBound({
        operation: "select",
        table: "run_reservations",
    ...
    expect(e.message).toContain("in a projection either");
  });

  it("leaves a table that binds no run projecting every declared column", () => {
    const c = build({ operation: "select", table: "idea_reservations" });
    expect(c.text).toContain('SELECT "id", "org_id", "idea_artifact_id", "state" FROM');
  });
...
describe("the scope a scope-bound table's rows belong to (cinatra#3249, per scope)", () => {
  ...
  it("fills the run AND the scope columns from the BOUND run on insertIfAbsent", () => {
    ...
    // The row the conflict keys stand under is looked for inside the bound
    // SCOPE, as it is looked for inside the organisation — and NOT inside the
    // bound run, which is no floor: the run only orders the two candidates.
    expect(c.text).toContain(
      'WHERE "org_id" = $7 AND "scope_kind" = $8 AND "scope_id" = $9 AND "id" = $10 ' +
        'ORDER BY ("run_id" = $11) DESC',
    );
  ...
  /**
   * THE SCOPE-WIDE RESERVATION (adopted from the convergence round). A pack
   * whose reservation belongs to the SCOPE declares its unique index across the
   * scope, so a second run's insert collides with the FIRST run's row. The
   * fallback that answers "the row that won" therefore may not be narrowed to
   * the inserting run: it would answer "not inserted, and no row" about a row
   * an ordinary select of the same scope hands straight back.
   */
  it("finds a SIBLING run's row as the row that won, inside the same scope", () => {
    const c = buildScopeBound({
      operation: "insertIfAbsent",
      table: "scope_reservations",
      row: { id: "r1", state: "reserved" },
    ...
    // The fallback's membership is the organisation and the scope, never the run.
    const fallback = c.text.slice(c.text.indexOf("UNION ALL"));
    const membership = fallback.slice(0, fallback.indexOf("ORDER BY"));
    expect(membership).toContain('WHERE "org_id" = $7 AND "scope_kind" = $8 AND "scope_id" = $9');
    expect(membership).not.toContain('"run_id" = $');
    // The run decides the ORDER instead, so this run's own row still wins when
    // both it and a sibling's satisfy the conflict keys.
    expect(fallback).toContain('ORDER BY ("run_id" = $11) DESC');
    expect(c.values.slice(6)).toEqual(["org-w7", "project", "project-77", "reserved", RUN]);
  });
  ...
  it("substitutes the bound scope on a select asking for THIS SCOPE'S rows", () => {
    ...
    expect(c.text).toBe(
      'SELECT "id", "org_id", "scope_kind", "scope_id", "state" FROM ' +
...
describe("the bound scope is a floor, not an option", () => {
  ...
  it("leaves a table that binds its rows to no scope compiling exactly as before", () => {
    ...
    expect(c.text).toBe(
      `SELECT "id", "org_id", "state" FROM "cinatra"."${RUN_BOUND_PHYSICAL}" ` +
        `WHERE "org_id" = $1 AND "state" = $2 LIMIT 100`,
    );
