...
describe("installWorkflowExtensionSaga — inverse-order compensating rollback", () => {
  it("on a WRITE-2 throw: archives dashboards THEN deletes the just-created template (in that order)", async () => {
    const h = makeHarness({
      materializeDashboardTemplate: async () => {
...
describe("installWorkflowExtensionSaga — idempotent finalize", () => {
  ...
  it("SEC-01: REQUIRE_SIGNATURES=true + unsigned → refuses BEFORE any writes (no template/dashboard, rolled back)", async () => {
