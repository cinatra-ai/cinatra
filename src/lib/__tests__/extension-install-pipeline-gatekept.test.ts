...
vi.mock("@/lib/deployment-registry-config", () => ({
  // BLOCKER-2(a): the final registry identity is sourced from the credential-free
...
describe("makeDefaultInstallPipelineDeps — flag ON (broker grant/proxy)", () => {
  ...
  it("resolveIntegrity fetches the packument THROUGH the broker grant but RETURNS the FINAL registry identity (not the broker URL) so trust classification stays correct", async () => {
    ...
    // BLOCKER-2(b): the returned registryUrl is the FINAL registry.cinatra.ai
  ...
  it("BLOCKER-2(a): the gatekept path NEVER calls loadVerdaccioConfigForServer (no server credentials required)", async () => {
    const deps = await makeDefaultInstallPipelineDeps();
  ...
  it("BLOCKER-2(b) end-to-end: a gatekept install of a SIGNED package stays TRUSTED (grant auto-approved) — trust classification sees registry.cinatra.ai, not the broker", async () => {
    // The full pipeline runs the REAL trust classifier (classifyExtensionTrust is
    // not mocked). Before the BLOCKER-2 fix, resolveIntegrity returned the broker
  ...
  it("BLOCKER-2(b) end-to-end: a gatekept install of an UNSIGNED package stays grant-PENDING — no auto-grant without a verified signature (capability split), regardless of scope", async () => {
    h.resolveGatekeptInstallConfig.mockResolvedValue({
