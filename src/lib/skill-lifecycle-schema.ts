...
// ---------------------------------------------------------------------------
// agent_assigned_skills (cinatra#2346 S1, epic #2345)
// ---------------------------------------------------------------------------
//
// The ACTOR-INDEPENDENT store of "this agent package uses these skills".
// Deliberately NOT `custom_skill_assignments`: that table's read is actor-gated
// (owner_type/owner_id filtered by the caller's principal, team, project and
// org), so an actor-less worker run never sees a row. A direct assignment made
// by an admin on an agent's settings page has to reach EVERY run of that agent,
// including background runs with no user attached — so the row carries no owner
// tuple at all.
//
...
// every route, so a new import there widens five route budgets the route-graph
