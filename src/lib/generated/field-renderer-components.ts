// @generated by scripts/extensions/generate-extension-manifest.mjs — DO NOT EDIT BY HAND.
// Regenerate: node scripts/extensions/generate-extension-manifest.mjs
// CONSUMED: the host's runtime source of truth for the connector surfaces —
// setup/settings-page loading, connector entry modules, and serverEntry
// activation resolve from these generated maps. The dev-watcher readdir scan
// remains the registration source for filesystem-loaded extension kinds;
// parity is checked against the connector catalog descriptors.
import type { ExtensionResolution } from "@cinatra-ai/sdk-extensions";

// Literal dynamic-import map (Turbopack rejects computed import templates).
// Consumed by the field-renderer component resolution wiring
// (packages/agents/src/field-renderer-components.ts) as the BUILD table of
// extension-shipped HITL field-renderer modules, keyed by the field-renderer
// BINDING id (own keyspace, separate from GENERATED_ARTIFACT_RENDERERS and
// from the neutral KIND table). A binding ABSENT from this map resolves to
// its host KIND component (behavior-preserving); a binding PRESENT here whose
// module is absent/degraded falls back to SchemaFieldRenderer (the never-blank
// floor). CLIENT-safe by construction: each loader is a plain literal dynamic
// import — the server-only guardedExtensionImport is intentionally NOT used
// (see the emitter header in the generator).
export type GeneratedFieldRendererComponentLoader = () => Promise<unknown>;

export type GeneratedFieldRendererComponentEntry = {
  resolution: ExtensionResolution;
  packageName: string;
  propsApiVersion: number;
  load: GeneratedFieldRendererComponentLoader;
};

export const GENERATED_FIELD_RENDERER_COMPONENTS: Record<string, GeneratedFieldRendererComponentEntry> = {
  "@cinatra-ai/auditor-agent:review": { resolution: "guardedOptional", packageName: "@cinatra-ai/auditor-agent", propsApiVersion: 1, load: () => import("@cinatra-ai/auditor-agent/src/auditor-review-renderer") },
  "@cinatra-ai/blog-linkedin-publish-agent:draft-review": { resolution: "guardedOptional", packageName: "@cinatra-ai/blog-linkedin-publish-agent", propsApiVersion: 1, load: () => import("@cinatra-ai/blog-linkedin-publish-agent/src/renderers/draft-review") },
  "@cinatra-ai/blog-wordpress-publish-agent:draft-confirm": { resolution: "guardedOptional", packageName: "@cinatra-ai/blog-wordpress-publish-agent", propsApiVersion: 1, load: () => import("@cinatra-ai/blog-wordpress-publish-agent/src/renderers/draft-confirm") },
  "@cinatra-ai/email-delivery-agent:output": { resolution: "guardedOptional", packageName: "@cinatra-ai/email-artifacts", propsApiVersion: 1, load: () => import("@cinatra-ai/email-artifacts/src/renderers/send-confirmation") },
  "@cinatra-ai/email-delivery-agent:send-confirmation": { resolution: "guardedOptional", packageName: "@cinatra-ai/email-artifacts", propsApiVersion: 1, load: () => import("@cinatra-ai/email-artifacts/src/renderers/send-confirmation") },
  "@cinatra-ai/email-drafting-agent:email-drafts-review": { resolution: "guardedOptional", packageName: "@cinatra-ai/email-artifacts", propsApiVersion: 1, load: () => import("@cinatra-ai/email-artifacts/src/renderers/email-drafts-review") },
  "@cinatra-ai/email-drafting-agent:follow-up-cadence": { resolution: "guardedOptional", packageName: "@cinatra-ai/email-artifacts", propsApiVersion: 1, load: () => import("@cinatra-ai/email-artifacts/src/renderers/follow-up-cadence") },
  "@cinatra-ai/email-follow-up-agent:email-drafts-review": { resolution: "guardedOptional", packageName: "@cinatra-ai/email-artifacts", propsApiVersion: 1, load: () => import("@cinatra-ai/email-artifacts/src/renderers/email-drafts-review") },
  "@cinatra-ai/email-follow-up-agent:follow-up-cadence": { resolution: "guardedOptional", packageName: "@cinatra-ai/email-artifacts", propsApiVersion: 1, load: () => import("@cinatra-ai/email-artifacts/src/renderers/follow-up-cadence") },
  "@cinatra-ai/email-outreach-agent:gmail-sender": { resolution: "guardedOptional", packageName: "@cinatra-ai/email-artifacts", propsApiVersion: 1, load: () => import("@cinatra-ai/email-artifacts/src/renderers/gmail-sender") },
  "@cinatra-ai/email-recipient-selection-agent:campaign-recipients-review": { resolution: "guardedOptional", packageName: "@cinatra-ai/email-artifacts", propsApiVersion: 1, load: () => import("@cinatra-ai/email-artifacts/src/renderers/campaign-recipients-review") },
  "@cinatra-ai/email-test-delivery-agent:input": { resolution: "guardedOptional", packageName: "@cinatra-ai/email-artifacts", propsApiVersion: 1, load: () => import("@cinatra-ai/email-artifacts/src/renderers/test-delivery-input") },
  "@cinatra-ai/list-curator-agent:final-list-review": { resolution: "guardedOptional", packageName: "@cinatra-ai/list-curator-agent", propsApiVersion: 1, load: () => import("@cinatra-ai/list-curator-agent/src/list-curator-final-list-renderer") },
  "@cinatra-ai/list-curator-agent:scrape-schema-review": { resolution: "guardedOptional", packageName: "@cinatra-ai/list-curator-agent", propsApiVersion: 1, load: () => import("@cinatra-ai/list-curator-agent/src/list-curator-scrape-schema-renderer") },
};
