// @generated by scripts/extensions/generate-extension-manifest.mjs — DO NOT EDIT BY HAND.
// Regenerate: node scripts/extensions/generate-extension-manifest.mjs
// Inbound-webhook registry metadata (one per cinatra.webhooks hook,
// cinatra#340). Import-free pure data for the #342 registry/nav UI — NO
// loaders (server loaders live in webhooks.server.ts). Inert until #343.
export type GeneratedWebhookRegistryMeta = {
  scope: string;
  vendor: string;
  slug: string;
  hook: string;
  label: string;
  rejectStatus?: number;
  schemaVersion?: number;
};

export const GENERATED_WEBHOOK_REGISTRY_META: readonly GeneratedWebhookRegistryMeta[] = [
  {"scope":"cinatra-ai/drupal-mcp-connector/node-published","vendor":"cinatra-ai","slug":"drupal-mcp-connector","hook":"node-published","label":"Node published","schemaVersion":1},
  {"scope":"cinatra-ai/wordpress-mcp-connector/post-published","vendor":"cinatra-ai","slug":"wordpress-mcp-connector","hook":"post-published","label":"Post published","schemaVersion":1},
];
