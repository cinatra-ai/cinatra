...
export const BACKGROUND_JOB_NAMES = {
  ...
  // D8 — durable repair for Twenty→cinatra pointer writes. When a
  ...
  try {
    switch (job.name as BackgroundJobName) {
      ...
      case BACKGROUND_JOB_NAMES.GRAPHITI_PROJECTION_REPAIR: {
        ...
        //
        // D8: register the Twenty provider + CRM sync adapter on every cycle
      ...
      case BACKGROUND_JOB_NAMES.TWENTY_POINTER_REPAIR: {
        // D8 durable-repair handler. One-shot per enqueue (NOT
