...
//     (D2: the runtime installer verifies npm/pacote SRI `dist.integrity` over
//     the EXACT downloaded tarball bytes BEFORE materialization);
//   - derive a digest-segment from the verified tarball;
//   - the bundled-dependencies gate (D4: runtime deps must be physically bundled
...
/**
 * D4: the materializer NEVER runs `npm`/`pnpm install` (no lifecycle scripts,
