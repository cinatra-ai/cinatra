...
// PM (project-management) provider discovery bridge.
//
// The host names no concrete PM provider package. A provider extension
// (plane today; linear/jira later) registers its `PmConnector` impl behind the
// `pm-provider` capability from its own `serverEntry`
// (`register(ctx)` → `ctx.capabilities.registerProvider("pm-provider", …)`).
// This module binds the SDK-hosted PM provider registry's EXTERNAL resolver to
// the host capability registry, so `lookupPmProvider(id)` resolves every
...
//
// Mirrors src/lib/register-crm-providers.ts exactly.
...
function isPmConnector(impl: unknown): impl is PmConnector {
  ...
  const candidate = impl as { providerId?: unknown; upsertRunTask?: unknown };
  return (
    typeof candidate.providerId === "string" &&
    candidate.providerId.length > 0 &&
    typeof candidate.upsertRunTask === "function"
  );
}
