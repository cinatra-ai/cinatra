...
export function resolveWidgetStreamOrigin(
  ...
  // cinatra#221: UNION the legacy instances[].siteUrl allowlist with the
  // non-revoked connect_sites.widgetOrigin allowlist. Same normalization. A
  // revoked site's origin is absent from this list, so revoke drops CORS
  // immediately. NOTE: origin-in-union alone does NOT authorize a cnx_ token —
  // validateConnectServerCredential additionally enforces the paired
  // Origin===site.widgetOrigin binding so one valid origin can never be a
  // confused deputy for another site's token.
  try {
