 ...
 * E2E). Lives in THIS leaf, beside the DDL for its own FK parent
 * (`assistant_turns`, created above), so the ordering requirement is structural
 * rather than a comment on a separate module a future edit could reorder — and
 * so the route graphs that reach `drizzle-store.ts` gain no module for it.
 *
 * The table is NET-NEW and purely ADDITIVE, so the idempotent bootstrap is the
 * whole story on both fresh installs and the operator upgrade path (it re-runs
 * at every boot) — no numbered migration, per migrations/README.md ("Additive
 * (no artifact required): A new table … New constraints and unique indexes
 * scoped to a table created in the same change").
 *
