...
/** The alias the conditional insert's fallback arm reads the blocking row under. */
const CONFLICT_ROW = "__conflicted";
...
export function buildExtensionDataStatement(input: {
  ...
  /**
   * THE RUN COLUMN IS NOT PROJECTED (cinatra#3249, adopted from the convergence
   * round). The organisation column is projected, and may be, because every row
   * a call can reach carries the ONE organisation the call is already inside;
   * the scope columns are the same, because the scope floor above stands on
   * every statement. The run column is NOT: the run is deliberately no floor,
   * so a scope-wide read reaches SIBLING runs' rows — and projecting the column
   * would hand the module those runs' identifiers, which is precisely the
   * identity the binding exists to keep out of a module's hands. So it is
   * dropped from the default projection and refused when a request names it,
   * and a module still says "mine" with the marker instead of reading an id.
   */
  const projectionColumns = (() => {
    if (!request.columns || request.columns.length === 0) {
      return table.columns.map((c) => c.name).filter((c) => c !== runCol);
    }
    for (const c of request.columns) {
      ...
      if (runCol && c === runCol) {
        throw new ExtensionDataRefusal(
          "run-is-the-hosts",
          `extension_data: the run column "${runCol}" is bound by the host — a request may not ` +
            `name it, in a projection either, and asks for this run's own rows with ` +
            `\`{ boundRun: true }\` in \`where\``,
        );
      }
    }
  ...
  if (request.operation === "insertIfAbsent") {
    ...
    /**
     * THE ROW THAT WON is looked for inside the organisation and inside the
     * bound SCOPE — the two floors every statement on this table already
     * stands on — and NOT inside the bound run (cinatra#3249, adopted from the
     * convergence round).
     *
     * The run is deliberately no floor, and a request can never name the run
     * column, so a pack whose reservation is the SCOPE'S — a row the next run
     * of the same project must not take again, which is the case the binding
     * exists for — declares its unique index across the scope. Its second run's
     * insert then collides with the first run's row, `ON CONFLICT DO NOTHING`
     * writes nothing, and a fallback narrowed to the inserting run would find
     * nothing either: the call would answer "not inserted, and no row" about a
     * row an ordinary select of the same scope hands straight back.
     *
     * A pack whose uniqueness IS per-run declares its index that way, and then
     * both runs' rows can satisfy this predicate at once. So the run decides
     * the ORDER rather than the membership: this run's own row is preferred,
     * and a sibling's is returned only when this run has none. Both index
     * shapes therefore answer with the row that actually blocked the insert.
     */
    const conflictWhere = [
    ...
    const conflictOrder = runCol ? ` ORDER BY (${qi(runCol)} = ${p(boundRun())}) DESC` : "";
    return {
      text:
        ...
        `UNION ALL ` +
        `SELECT false AS ${qi(INSERTED_FLAG)}, ${distinctProjection} FROM ` +
        `(SELECT * FROM ${target} WHERE ${conflictWhere}${conflictOrder} LIMIT 1) ` +
        `AS ${qi(CONFLICT_ROW)} ` +
        `WHERE NOT EXISTS (SELECT 1 FROM ${qi(ATTEMPT_CTE)}) ` +
        `LIMIT 1`,
      values,
      physicalTable: physical,
      rowKeys,
