...
export const SYNC_CALLER_CLASSIFICATIONS: Record<string, SyncCallerClassification> = {
  ...
  "src/lib/artifacts/typed-promotion-store.ts": {
    class: "migratable-request-path",
    justification:
      "The typed promotion road's store half (cinatra#3028, epic #3023 — enabler 0.14). Four sync call sites, all on the library's Confirm request path: the base-typed row read (with its latest representation and the blob's own detected mime, because the promotion re-validates the shared content against the target type's accepted forms), the matcher-association read, the PERSON'S-OWN-ASSERTION read (the authority that outranks the matcher must be proved from the durable row, never believed from a caller flag, and a converging re-run writes no second assertion so only the row can prove it), and the append-only representation revision the promotion adds over the base revision's own resource — written with its artifact-writer witness in the same advisory-locked transaction, because a promoted row is a claimed pack-typed row whose bytes no read path admits without one. The retype itself is NOT here — it goes through the canonical history-aware objects writer. Sync leaf mirroring representation-store / semantic-assertion-store so it composes into the synchronous store graph; migratable to async pooled access with the artifacts subsystem.",
  },
