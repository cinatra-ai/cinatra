  ...
  return (
    <button
      ...
      className={cn(
        ...
        // The rail toggles collapse/expand on click (it does not drag-resize),
        // so use a pointer cursor — a resize cursor falsely implies dragging.
        "cursor-pointer",
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        "flex h-8 shrink-0 items-center rounded-md px-3 pt-2 pb-1 text-[10px] font-semibold uppercase tracking-kicker text-sidebar-foreground/70 ring-sidebar-ring outline-hidden transition-[margin,opacity] duration-200 ease-linear group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0 focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
        className
      )}
      {...props}
    />
