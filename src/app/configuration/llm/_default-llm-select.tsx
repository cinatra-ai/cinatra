...
export function DefaultProvidersCard({
  ...
  return (
    <>
      ...
      {/* The Anthropic skill-upload governance section (the "Upload skill
          content to Anthropic" opt-in + its non-ZDR data-residency warning, and
          the not-connected "Connect Anthropic" affordance) moved OUT of core
          (cinatra#1104, the S3a removal half) into the anthropic-connector's own
          Skills tab (the S3b half). The
          setting is now rendered + written by the connector (through the
          `@cinatra-ai/host:anthropic-skill-config` capability); core retains the
          canonical reader + that host-capability writer the ~7 consumers and the
          connector share. */}
