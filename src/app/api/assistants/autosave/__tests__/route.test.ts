...
// ---------------------------------------------------------------------------
// Route-handler regression test for the first-class chat-capture ("autosave")
// config route on the assistants surface (cinatra#1218):
// GET/PATCH /api/assistants/autosave. The authz kernel runs for real; the
// session, the skill-autosave store, and the audit sink are mocked. Asserts the
// SAME gate as the legacy /api/chat/autosave: GET needs a session (401), PATCH
// admin arm is platform-admin only (401/403), cross-origin -> 403 before auth,
// audit-fail -> 503, and the self arm writes only the caller's own preference.
// ---------------------------------------------------------------------------
...
const ENDPOINT = "https://app.test/api/assistants/autosave";
...
describe("assistants/autosave route handler (chat-capture config gate)", () => {
  ...
  it("PATCH self arm 403 for a non-admin while userCanConfigure is off — pref never written", async () => {
    getActorContext.mockResolvedValue(orgAdmin());
    const { PATCH } = await import("../route");
  ...
  it("PATCH self arm allows a platform admin; null resets to follow-default", async () => {
    getActorContext.mockResolvedValue(platformAdmin());
    const { PATCH } = await import("../route");
