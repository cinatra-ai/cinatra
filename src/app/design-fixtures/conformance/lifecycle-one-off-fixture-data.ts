...
// one thing the drawing says once. Five of them are addressable on the default
// branch today and are mounted by `lifecycle-one-off-fixtures.tsx` from the
// SHIPPED components; the other seven are recorded on the wave's
...
/**
 * The three rows of §IX's reader matrix — what the READER may do, which is what
 * holds a card back or lets it through. They are drawn by handing the shipped
 * component the three different inputs a real host hands it, never by styling
 * one reading to look like another.
 */
export const LIFECYCLE_READER_STATES = [
  "may-view-and-act",
  "may-view-not-act",
  "may-not-read",
] as const;
...
/**
 * The one surfaced suggestion the §IX matrices draw a card with. The chips are
 * the piece of the review card a harness can mount as the product mounts it
 * (cinatra#3156), so they are what the two matrices carry: the matrices fix
 * PRESENCE and the reader's reading, not which card is in the cell.
 */
