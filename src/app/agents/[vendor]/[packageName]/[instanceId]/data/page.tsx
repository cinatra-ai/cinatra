...
export default async function AgentPackageInstanceDataPage({ params, searchParams }: Props) {
  const { vendor, packageName, instanceId } = await params;
  ...
  if (!screens) notFound();
  if (!("instanceData" in screens) || !screens.instanceData) notFound();
  return (screens.instanceData as (props: { agentId: string; instanceId: string; searchParams?: typeof searchParams }) => Promise<React.ReactNode>)({ agentId, instanceId, searchParams });
}
