...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return resolveAgentInstanceMetadata({ vendor, packageName, instanceId, subRoute: "optimization" });
}
...
export default async function AgentPackageInstanceOptimizationPage({ params, searchParams }: Props) {
  const { vendor, packageName, instanceId } = await params;
  ...
  if (!screens) notFound();
  if (!("instanceOptimization" in screens) || !screens.instanceOptimization) notFound();
  return (screens.instanceOptimization as (props: { agentId: string; instanceId: string; searchParams?: typeof searchParams }) => Promise<React.ReactNode>)({ agentId, instanceId, searchParams });
}
