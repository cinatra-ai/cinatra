...
describe("generic /webhook route — a legacyEnabled binding fails closed (cinatra#2022)", () => {
  // cinatra#2022 deleted the #343 legacy `sha256=<hex>` HMAC bridge. A
  // `legacyEnabled` binding (a pre-cinatra#2022 row, if any still exists)
  // resolves with an empty candidate-secret list, so it now fails the same
  // Standard-Webhooks verification path as any other unverifiable binding —
  // there is no separate legacy branch left to exercise.
  it("a legacyEnabled binding with empty secrets → 401 (no dispatch)", async () => {
    resolveByBindingId.mockResolvedValue(
      baseBinding({ legacyEnabled: true, secrets: [] }),
    );
    const res = await POST(signedRequest({ a: 1 }), params());
    expect(res.status).toBe(401);
    expect(claim).not.toHaveBeenCalled();
