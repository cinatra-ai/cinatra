 ...
 *
 *   1. the report exists and parses      — a runner that crashed before writing
 *                                          one never "passed";
 *   2. `success === true`                — belt-and-braces beside the runner's
 *                                          exit code;
 *   3. the tier contract is non-empty    — an `--expect` glob matching nothing
 *                                          makes the check below vacuous, so it
 *                                          would pass whatever ran;
 *   4. at least one file executed        — `vitest run` over a pattern that
 *                                          matches nothing is the canonical
 *                                          silent green;
 *   5. every file matching the contract  — the anti-recurrence clause. A config
 *      was actually executed               `include` that narrows, a stray
 *                                          `exclude`, or a new test file landing
 *                                          outside the runner's reach all red
 *                                          here.
 *
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    const take = () => {
      ...
      // Reject an empty value or the next OPTION token. `--expect --title x`
      // would otherwise record "--title" as the tier contract; combined with an
      // expectation set that then matches nothing, the guard would pass while
      // proving nothing — the very outcome it exists to prevent.
      // (Codex round 2, SHOULD-FIX.)
      if (value === undefined || value === "" || value.startsWith("--")) {
        throw new Error(`Missing or malformed value for ${arg} (got: ${value ?? "<end of args>"})`);
      }
      i += 1;
      return value;
    };
...
/** Package-relative, POSIX-separated — the one shape every comparison here uses. */
...
function main() {
  ...
  // --- the tier contract: what SHOULD have run (checks 3 and 5 below) -----
  ...
  // --- 3. the tier contract itself must not be empty -----------------------
  // An empty EXPECTED set makes the coverage check vacuous: `missing` is [] no
  // matter what ran, so a report containing one unrelated file would pass. That
  // happens for a typo'd `--expect`, a renamed directory, or exclusions that
  // swallow every match — each of which means the tier is no longer being
  // checked, which is indistinguishable from the tier not existing.
  // (Codex round 2, MUST-FIX.)
  if (expected.size === 0) {
    fail(
      `${args.title}: the tier contract matched ZERO files on disk ` +
        `(--expect ${args.expect.map((g) => `'${g}'`).join(" ")}` +
        (args.exclude.length > 0
          ? `, --exclude ${args.exclude.map((g) => `'${g}'`).join(" ")}`
          : "") +
        "). An empty expectation makes this guard vacuous — it would pass whatever ran. " +
        "Fix the glob or the tier's location; do not leave the contract empty.",
    );
    return;
  }

  // --- 4. something actually ran -------------------------------------------
  if (executed.size === 0) {
    fail(
  ...
  // --- 5. everything the contract names actually ran ------------------------
  if (missing.length > 0) {
    fail(
