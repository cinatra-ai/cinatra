...
test("orchestrator declares exactly the ten designed arms", () => {
  ...
  // six fresh-init arms (cinatra#352) + the four upgrade-from arms (cinatra#1421/#1422)
  assert.deepEqual(
    arms,
    ["graphiti", "nango", "postgres", "redis", "verdaccio", "wayflow", "upgrade-redis", "upgrade-mariadb", "upgrade-postgres", "upgrade-neo4j"].sort(),
  );
});

test("each arm script exists and is referenced by the orchestrator", () => {
  const armsDir = resolve(REPO_ROOT, "scripts/ci/works-after");
  for (const arm of ["redis", "verdaccio", "nango", "wayflow", "graphiti", "postgres", "upgrade-redis", "upgrade-mariadb", "upgrade-postgres", "upgrade-neo4j"]) {
