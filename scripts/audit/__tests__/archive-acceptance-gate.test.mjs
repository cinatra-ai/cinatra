...
describe("strictVerdict — the FULL strict-mode pass condition", () => {
  ...
  it("the real committed manifest is honestly NOT READY under strict mode today (cinatra#1943 A1/A2/A3/A3b/A4/A5 landed 6 rows; A6/A7 remain) — not a bug", () => {
    ...
    // BA-DML-vs-archive both lock interleavings (A4), and Dual-transport
    // coverage (A5) are structurally verified green this stage; every other
    // row honestly stays red pending its own sibling gate or CI-wiring PR.
    expect(verdict.notYetGreen).toHaveLength(9);
    expect(verdict.rowFailures).toEqual([]);
  });
});
