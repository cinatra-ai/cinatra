...
// `release: published` workflow (only publishing a GitHub Release does — see the
// runbook), so running this tool cannot publish anything. Tagging, releasing,
// marketplace republish, and the destructive unpublish of higher versions are
// the OWNER-gated steps 2–4 and are intentionally OUT OF SCOPE here.
//
...
export function assertNoPublishingOps(tool, args) {
  ...
  if (tool === "git") {
    if (a.includes("tag")) throw new Error("guard: this tool must never `git tag` (tagging is owner-gated step 2)");
    if (a.includes("push")) {
      for (const x of a) {
  ...
  if (tool === "gh") {
    if (a[0] === "release") throw new Error("guard: this tool must never `gh release` (releasing is owner-gated step 3)");
...
async function fetchRepoVersions(only) {
  ...
  // `.github/workflows/release.yml` caller (the same marker the org-token runbook
  // uses). An explicit --only list is trusted as-is.
  let names;
  if (only) {
    names = only;
  } else {
...
async function main() {
  ...
  if (!apply) {
    ...
    process.stderr.write("This tool never tags, releases, or publishes — those are the owner-gated steps 2–4.\n");
    return;
  }
