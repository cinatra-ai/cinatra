...
// (cinatra#1628, S11b). CARRIER-KIND GATED: only `kind:"artifact"` may author the
// claim (the sole allowlisted carrier per the sdk leaf — re-homed off the retired
// `agent` carrier by cinatra#1896 / epic #1883), so any other kind emits null even
...
export async function buildManifest() {
  ...
  const records = inv.extensions.map((x) => {
    ...
    return {
      ...
      // The type ids an artifact pack declares in `cinatra.artifact.objectTypes`
      // (cinatra#3033), read for the claims map only; empty for every other kind.
      artifactObjectTypeIds:
        x.kind === "artifact" && Array.isArray(cin.artifact?.objectTypes)
          ? cin.artifact.objectTypes
              .map((t) => (isObj(t) ? t.type : undefined))
      ...
      // re-homed to the artifact kind by cinatra#1896 / epic #1883): the versioned
      // dashboard-contribution claim, emitted ONLY on kind:"artifact" records (the
      // sole carrier kind per the sdk leaf allowlist — the `agent` carrier is
      // retired) — null on every other kind, and null when the pack declares none.
      // Carried UNVALIDATED (same pattern as accessConfig/envOverrides); the host
      // parses it fail-closed + field-tolerantly via `parseDashboardContribution`
      // at consumption (the S11b adoption reconciler), degrade-with-diagnostic — so
      // a malformed claim can never reject the surrounding artifact manifest. INERT
      // until a meaning pack ships the claim (no bundled extension declares it
  ...
  // The "artifact-default-floor" role + its generated floor-claimant constant
  // were retired with @cinatra-ai/default-artifact (epic cinatra#1785 wave A5):
  // there is no longer a structural fallback artifact type, so no role claim is
  // validated or emitted here. `cinatra.roles` declarations are still merged
...
// The packs' own CROSS-NAMESPACE claims (cinatra#3033): every declared type id
// whose package part differs from the declaring record's own package, kept
// ONLY when exactly one present `kind:"artifact"` record declares that id,
// keyed by the type id and sorted by it so the emission is byte-stable.
export function artifactKindClaimEntries(records) {
  const declarers = new Map();
  for (const r of records) {
    if (r.kind !== "artifact" || !Array.isArray(r.artifactObjectTypeIds)) continue;
    for (const typeId of new Set(r.artifactObjectTypeIds)) {
      if (typeof typeId !== "string" || !typeId.includes(":")) continue;
      const list = declarers.get(typeId) ?? [];
      list.push(r.packageName);
      declarers.set(typeId, list);
    }
  }
  return [...declarers]
    .filter(([typeId, pkgs]) => pkgs.length === 1 && typeId.split(":")[0] !== pkgs[0])
    .map(([typeId, pkgs]) => ({ typeId, packageName: pkgs[0] }))
...
export function emitArtifactKindLabels(records) {
  ...
  const body = artifactKindLabelEntries(records)
    ...
    .join("\n");
  const claims = artifactKindClaimEntries(records)
    .map((e) => `  ${JSON.stringify(e.typeId)}: ${JSON.stringify(e.packageName)},`)
    .join("\n");
  return (
    `// @generated by ${script} — DO NOT EDIT BY HAND.\n` +
    `// Regenerate: node ${script}\n` +
