...
test.describe("§V detail modal — chrome/header tokens (no-regression composition)", () => {
  test("720px chrome, 64px radius-15 tile, right-aligned price, no footer (cinatra#2406)", async ({
    page,
  }) => {
    const modal = await openModal(page, "modal-fixture-populated");
    ...
    // cinatra#2406 (owner ruling): no footer — and so no "Install now" /
    // "Update now" button — renders anywhere in the modal.
    await expect(modal.getByRole("button", { name: "Install now" })).toHaveCount(0);
    await expect(modal.getByRole("button", { name: "Update now" })).toHaveCount(0);
