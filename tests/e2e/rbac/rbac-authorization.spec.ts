/**
 * RBAC browser authorization suite (RBAC-UI-01..04).
 *
 * Scenarios (representative of the documented authorization flows):
 *   1. RBAC-UI-02: nav visibility — a non-admin member does NOT see the
 *      admin-only "Analytics" nav entry.
 *   2. RBAC-UI-01: the project permissions surface renders the
 *      access-vs-ownership clarity note (project seeded by auth.setup.ts).
 *   3. RBAC-UI-04: role-gated admin surface — a member hitting
 *      /configuration/access-control is denied.
 *   4. RBAC-UI-03: single-org mode — when the instance toggle is on, the
 ...
 *   5. RBAC-EXT-01: project admin can grant a customer and revoke them
 *      (uses the customer user seeded by auth.setup.ts).
 *
 * The EXT-02 customer-scoped view runs in a separate spec
...
// 180s overall — EXT-01 carries 60s invite + 60s revoke assertions plus
// navigation/hydration; the 120s default is too tight on cold CI.
...
test.describe("RBAC — nav visibility + access clarity + role gate", () => {
  test("RBAC-UI-02: member does not see the admin-only Analytics nav entry", async ({ page }) => {
  ...
  test("RBAC-UI-01: project permissions surface shows the ownership/access clarity note", async ({ page }) => {
  ...
  test("RBAC-UI-04: non-admin is denied the Access Control admin surface", async ({ page }) => {
...
test.describe("RBAC-UI-03: single-org mode", () => {
...
test.describe("RBAC-EXT-01: project admin grant → revoke customer", () => {
