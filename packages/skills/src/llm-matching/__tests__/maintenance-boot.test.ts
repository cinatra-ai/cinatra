/**
 * Match-store boot env-gating unit tests. The opt-in maintenance tick
 * (SKILL_MATCH_MAINTENANCE_CRON, #1365) gates on an env var (no DB column → no
 * migration): `resolveMaintenanceCron` is the pure gate — unset or invalid =>
 * disabled (null); a valid 5/6-field cron => enabled.
 */
