 ...
 *
 * Mocks `@/lib/agent-assigned-skills-store` wholesale (the sweep's own logic
 * is covered by `src/lib/__tests__/agent-assigned-skills-store-teardown.test.ts`)
 * so this file stays focused on ORDERING and the packageId that reaches the
 * sweep.
 */
