...
// epic #2345). Folded into this module (rather than a dedicated file) so the
// route-graph ratchet gate does not see a new node on every locked route
// this module is already reachable from — `uninstallSkillPackage` below is
// this slice's only first-party consumer.
//
// S1 gave the platform somewhere to remember an assignment; this section is
...
//
//   1. SKILL-package uninstall (`uninstallSkillPackage` below) sweeps every
//      `agent_assigned_skills` row keyed on this package's EXACT DERIVED
//      catalog ids — via the SAME `deriveSkillRegistration` derivation the S1
//      predicate's `buildSkillIdOwnership` uses, so the virtual
//      `@cinatra-ai/chat` namespace (the five chat successor packages) is
//      swept correctly. A virtual-namespace registration may have NO native
//      `skillPackages` catalog row at all, so this sweep is driven off the
//      REAL npm package name — recovered from the catalog packageId per
//      `skill-package-source.ts`'s own invertible `verdaccio:<name>` /
//      `github:<name>` contract — never off a native catalog lookup that
//      could legitimately come back empty.
...
// the failure: the co-owner cleanup in `uninstallSkillPackage` (cinatra#2346/
// #300) is the precedent — if this cleanup fails, the whole uninstall must
// roll back rather than silently leave an orphan assignment that could
// re-apply on a later reinstall of the same package/agent.
// ---------------------------------------------------------------------------
...
//     `./skills-store` (this module) — a static import here would close a
//     genuine load-time cycle, not just a lint nit. This module already
//     dodges the same shape of cycle with `./github` via
//     `tryAutoSyncConfiguredRepository` above.
...
async function defaultTeardownScanExtensions(): Promise<SkillExtensionDescriptor[]> {
...
async function defaultDeleteAssignedSkillsBySkillIds(
  skillIds: string[],
 ...
 * called by `uninstallSkillPackage` below BEFORE its missing-native-package
 * early return. `packageId` is the catalog packageId (`verdaccio:<name>` /
 ...
 * — a virtual-namespace registration may have no row there at all.
 */
export async function sweepAssignedSkillsForSkillPackageId(
  ...
  const deleteBySkillIds = deps.deleteBySkillIds ?? defaultDeleteAssignedSkillsBySkillIds;
...
// Type-only — erased at compile time, so this does NOT close the runtime
// skills-store <-> extension-skill-resolver cycle (that module imports
// several VALUE exports from this one). See the lifecycle-teardown section
// below (cinatra#2350 S5) for why the actual scan/derive calls stay dynamic.
