// core__0061 assistant registry-foundation (cinatra#1874 Epic #1873 W1) —
...
const mod = await import(
  path.join(REPO_ROOT, "migrations", "core", "core__0061_assistant-registry-foundation.mjs")
);
...
describe("core__0061 — module shape", () => {
  ...
  it("ships its append-only ledger fragment (union ledger seq 0061, non-destructive)", () => {
    const { entries, errors } = readManifestUnion(path.join(REPO_ROOT, "migrations"));
    expect(errors).toEqual([]);
    const entry = entries.find((m) => m.seq === "0061");
    expect(entry).toBeDefined();
    expect(entry?.file).toBe("core/core__0061_assistant-registry-foundation.mjs");
...
describe("core__0061 — up SQL shape", () => {
  ...
  it("existence-guards the backfill on all three referenced tables", () => {
    const bf = mod.buildAssistantHandlesOriginSql().find((s) => s.includes("core0061bf"));
