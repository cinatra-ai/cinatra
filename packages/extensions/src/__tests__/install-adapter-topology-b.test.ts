...
//
// Per LD-05 locked decision: "do not let same-interface framing erase the difference."
...
//
// Per CF-01: publish token decrypted with aad "destination.<id>.publish-token";
...
describe("resolveInstallEnvironment — topology B (shared-acl)", () => {
  ...
  // -------------------------------------------------------------------------
  // Per-field AAD binding (CF-01)
  // -------------------------------------------------------------------------

  it("decrypts publish token with AAD 'destination.<id>.publish-token' when readToken absent (CF-01)", async () => {
  ...
  it("decrypts read token with AAD 'destination.<id>.read-token' when readToken present (CF-01)", async () => {
