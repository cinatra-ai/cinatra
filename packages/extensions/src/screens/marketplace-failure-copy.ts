...
/**
 * What a marketplace lifecycle FORM action returns to the client on FAILURE.
 * A success never returns (it `redirect()`s), so a returned value always means
 * the operation did not take.
 */
export type MarketplaceInstallActionResult = MarketplaceInstallFailureResult;
