// Public field-renderer PROPS CONTRACT (cinatra#1625, epic #1620 S8 — M3).
//
// The STABLE PUBLIC MIRROR of the normalized props every HITL field renderer
// receives. This is the single source of truth: a claiming extension that ships
// a migrated renderer imports its props type from
// `@cinatra-ai/sdk-ui/field-renderer-props` (an agent extension may import ONLY
// `@cinatra-ai/sdk-extensions` + `@cinatra-ai/sdk-ui` as first-party code deps —
// the host-internal `@cinatra-ai/agents` is NOT resolvable from an extension),
// and the host re-exports these same types from
// `@cinatra-ai/agents` (packages/agents/src/field-renderer-registry.ts) so the
// ~34 existing in-core importers keep resolving unchanged.
//
// ZERO-RUNTIME except the ABI version constant: these are pure structural types
// (no React, no host imports) so the leaf is safe to import from both the lean
// design package and any extension bundle.
//
// VERSIONING: `FIELD_RENDERER_PROPS_API_VERSION` is the props-ABI generation
// checked at cutover — a migrated renderer declares `component.propsApiVersion`
// in its `cinatra.fieldRenderers` manifest entry; the host resolution degrades a
// binding whose declared version differs from this host constant to the
// `SchemaFieldRenderer` floor (abi-incompatible). This is the ONE authority for
// that integer; `packages/agents/src/field-renderer-components.ts` re-exports it
// (it must not hold a second copy). BUMP THIS ONLY on a BREAKING change to the
// `FieldRendererProps` shape (a removed/renamed field, a changed callback
// signature, or a narrowed required set) — a purely additive OPTIONAL field is
// backward-compatible and does NOT require a bump.

/** The props-contract ABI generation. Bump only on a breaking FieldRendererProps change. */
