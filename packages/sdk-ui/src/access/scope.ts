...
// The token vocabulary lives one directory up, in this same package: these
// helpers and the picker that mounts them ship together, so neither reaches
// the agent core. The agent core re-exports the same three names.
import {
  normalizeVisibilitySelection,
  type AccessVisibility as AgentAuthPolicyVisibility,
  type AccessVisibilitySelection as AgentAuthPolicyVisibilitySelection,
...
// `resolveAccessLabel` (packages/sdk-ui/src/access-combobox.tsx) delegate here, so
