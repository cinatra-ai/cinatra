...
// `cinatra.artifact.ui` renderer's default-export component receives. Re-exported
// here (and at the subpath) so an extension's renderer types its props against the
// SDK — never a host-internal import. Host-neutral MIRROR of the host source of
// truth (`src/lib/artifacts/artifact-renderer-props.ts`); a host-side
// mutual-assignability test keeps the two in lockstep.
export type { ArtifactRendererProps } from "./artifact-renderer-props";
export { ARTIFACT_RENDERER_PROPS_API_VERSION } from "./artifact-renderer-props";
