...
describe("pm-provider-registry-contract — SDK-hosted provider registry", () => {
  ...
  it("shares the registry across SEPARATE module instances (globalThis-anchored)", async () => {
    ...
    // and the bridge looks it up in ANOTHER bundle. Simulate distinct module
    // instances with vi.resetModules() + a fresh dynamic import — a
    // module-LOCAL Map would make the fresh instance's lookup miss.
    registerPmProvider(fakeProvider("plane"));
    vi.resetModules();
