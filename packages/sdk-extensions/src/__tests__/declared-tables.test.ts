...
describe("the run column a declaration may name (cinatra#3249)", () => {
  ...
  it("refuses a run column the host's own value could never be written into", () => {
    // The host writes a run id — text — exactly as it writes the organisation.
    // A column typed anything else is refused at the declaration, not at the
    // first insert, where it would surface as a database type error.
    const bad = [
      {
        ...RUN_BOUND[0],
        columns: RUN_BOUND[0].columns.map((c) =>
          c.name === "run_id" ? { name: "run_id", type: "integer", notNull: true } : c,
        ),
      },
    ];
    expect(() => parseDeclaredTables(bad, PIPELINE)).toThrow(/must be declared `text`/);
  });
});
...
describe("the scope columns a declaration may name (cinatra#3249)", () => {
  ...
  it("refuses a scope column the host's own values could never be written into", () => {
    // The launch anchor's kind and its id are both text, and the workspace arm
    // carries a text sentinel for the id. A column typed anything else — an
    // `integer` kind, a `uuid` id that no sentinel fits — is refused here.
    const typed = (name: string, type: string) => [
      {
        ...SCOPE_BOUND[0],
        columns: SCOPE_BOUND[0].columns.map((c) =>
          c.name === name ? { name, type, notNull: true } : c,
        ),
      },
    ];
    expect(() => parseDeclaredTables(typed("scope_kind", "integer"), PIPELINE)).toThrow(
      /must be declared `text`/,
    );
    expect(() => parseDeclaredTables(typed("scope_id", "uuid"), PIPELINE)).toThrow(
      /must be declared `text`/,
    );
  });
