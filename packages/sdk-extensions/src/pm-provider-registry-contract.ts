// Provider-agnostic PM (project-management) provider REGISTRY — lives in the SDK.
//
...
// @cinatra-ai/sdk-extensions, avoiding a coupling edge where every PM provider
// extension (plane-connector) would import a host package by name to
// self-register.
//
// The host wiring is ADDITIVE: src/lib/register-pm-providers.ts binds this
// registry's EXTERNAL resolver to the host capability registry, so a PM provider
// that self-registered behind the `pm-provider` capability resolves through
// `lookupPmProvider(id)` without the host naming the connector package.
//
// The Map is anchored on `globalThis` via a namespaced+versioned Symbol so the
// provider's boot-time registration and the bridge's lookup resolve the SAME Map
...
// cross-compilation reasoning as the crm-provider + action-guard DI contracts.
...
// cross-compilation reason as the Map above. Uses the shared
// `createHostDepsSlot` primitive (a single nullable value); the slot identity
...
/**
 * Bind (or clear) the host's lazy external PM-provider resolver. Called once
...
/**
 * Register a concrete PM provider. Called at boot (host register-pm-providers.ts
 * → the provider extension's register(ctx) capability registration, surfaced
 * via the external resolver — or a direct registration in tests).
