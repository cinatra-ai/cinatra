  ...
  switch (body.outcome) {
    ...
    case "stale":
      return typeof body.latestRevisionId === "string" &&
        ...
        : { outcome: "failed", reason: "malformed-answer" };
    case "refused":
      return typeof body.reason === "string" && body.reason in REFUSAL_SENTENCES
...
export async function saveArtifactEdit(
  ...
  try {
    response = await send(capability.saveUrl, {
      ...
      body: JSON.stringify(buildArtifactEditRequest(capability, text)),
  ...
  try {
    ...
    // A status with no readable body still has to become an outcome. A refusal
    // status reads as a refusal the host did not spell out; anything else is a
    // failure. Either way the indicator keeps the spinner.
    return response.status === 403
      ? { outcome: "refused", reason: "no-write-rights" }
      : { outcome: "failed", reason: response.ok ? "malformed-answer" : "server" };
  }
