...
//   - cinatra-dev-extensions.lock.json — every OTHER cinatraDevExtensions
 ...
 *   - a dev-lock entry is not in `cinatraDevExtensions` (stale pin);
...
export function loadDevExtensionPins(repoRoot, readFile = readFileSync) {
  ...
  if (!config || Object.keys(config).length === 0) {
    throw new Error("[cinatra dev-extensions] pinned sync requires a non-empty `cinatraDevExtensions` config.");
  }
    ...
    if (!(p.packageName in config)) {
      throw new Error(
        `[cinatra dev-extensions] ${DEV_EXTENSIONS_LOCK_FILENAME} pins "${p.packageName}", which is not a ` +
          `cinatraDevExtensions entry — stale pin; regenerate the dev lock.`,
      );
    }
