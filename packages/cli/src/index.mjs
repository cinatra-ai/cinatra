...
async function runSetup(mode, { skipDevApps = false } = {}) {
  ...
  try {
    ...
    if (mode === "dev") {
      ...
      // the extracted repos (cinatra-ai/{wordpress-plugin,drupal-module}).
      ...
      // Clone the extracted extension repos (cinatra-ai/<slug>) into
      // extensions/<scope>/<name>. Source of truth is the extracted repos
...
async function runSetupBranch(argv) {
  ...
  // 6c. Sync the extracted extension repos into THIS worktree (no-op until
...
async function runSetupClone(argv) {
  ...
  // Sync the extracted extension repos into THIS clone worktree (no-op until
  // `cinatraDevExtensions` is populated). Same loud-but-non-fatal posture.
  let extensionSync;
  try {
    extensionSync = await syncCinatraDevExtensions({
...
// `.claude/worktrees/...` (light branch) entries are skipped entirely so
...
function pruneCliOwnedWorktree(slug, recordedWorktreePath) {
  ...
  if (recorded !== expectedSibling) {
    // Unmanaged worktree (e.g. .claude/worktrees/...) — DB/slot/
...
async function runCloneStart(argv) {
  ...
  try {
    ...
    // URL from `Self.DNSName`, fail loud on a MagicDNS collision (D3),
...
async function runClonePruneStale(argv) {
  ...
  for (const slug of staleSlugs) {
    try {
      ...
      // and no behavior change for `.claude/worktrees/...` slots.
