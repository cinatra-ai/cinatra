...
`isTrustedDevPeer` and `shouldGrantDevAdminBypass` are pure helpers — keep them that way. `src/dev-admin-bypass-request.ts` is the ONE request-level composition, and both consumers resolve to it, so the two surfaces cannot drift into two trust boundaries. `src/lib/cli-api/route-guard.ts` calls `grantDevAdminBypassForRequest` directly. `index.tsx` may NOT: it is the package entry (`.`), so anything it imports lands in the graph of every call site that imports the package for something else, and the composition needs a credential-file reader plus a `node:http` capture. It therefore reads the PORT in `src/dev-admin-bypass.ts`, which the app's boot hook fills with that same composition via `installDevAdminBypassRequestPort()` — the hook that already installs the capture and mints the credential. An unfilled port refuses, which is what a process without that hook would answer anyway. The port does not take the installed function's word for the two guards that make this development-only: it applies `NODE_ENV !== "production"` and the opt-in flag itself, refuses to install anything at all when they do not hold, and is write-ONCE, so nothing loaded after the boot hook can put a wider answer behind it. Keep the readers off `index.tsx`'s graph; `src/__tests__/dev-admin-bypass-transport-graph.test.ts` fails if they come back. On the bypass path the OAuth verify is skipped, so `bearerSignatureVerified` is false and every header-derived identity arm in `actor-identity.ts` stays shut: the caller is the anonymous local operator, never a named user. The credential header is in the MCP log redaction set (`src/lib/mcp-logging.ts`) — an enabled server log writes every request header to disk, so leaving it out would copy the `0600` credential into a plain-text file. The A2A_DEV_BYPASS org fallback (`index.tsx`) is a SEPARATE opt-in with its own switch and is untouched here.
