...
3. Start a run: call `agent_run` with the required inputs. A normal dispatch returns `{ runId, status: "queued" }`. A chat-origin run can instead come back `{ runId, status: "pending_input" }`, held on the in-chat recommendation card, paused for the person to Confirm or Skip before it starts. (A request that fails validation or preflight returns a structured error instead of either.)
4. Poll for completion: for a `queued` run, call `agent_run_get` with the `runId` every 3-5 seconds. Stop when `status` is `"completed"`, `"failed"`, `"stopped"`, `"pending_approval"`, or `"pending_input"`. `pending_input` is an OVERLOADED status: reached here, from a run that started `queued`, it means the run paused on a different, mid-execution human-wait gate — NOT the recommendation card, which only ever arrives as the initial `agent_run` result in step 3, never from a later poll. Report it as paused and needing attention without guessing the gate. For a run that returned `pending_input` at dispatch, do not poll it at all and do not report it as running — it has not started, and only the person's Confirm or Skip on the card changes that. Once they decide, the run proceeds, but nothing notifies you: call `agent_run_get` again yourself if you are still in the conversation.
