...
type CanonicalRow = {
  ...
  // D8: `source` gates projection to cinatra-originated writes (agent | ui);
...
export async function projectObjectToGraphiti(input: {
  ...
  // D8 — source gate. Graphiti indexes data ORIGINATED in cinatra (agent
  ...
  // failed), no retry. `null` source predates D8 and is treated as cinatra-
  // originated to avoid dropping legacy rows.
  ...
  // D8 — adapter routing. Adapter-owned types (CRM account/contact under
  ...
  if (adapters.length > 0) {
    ...
    // Adapters are configless built-ins for now (D8 ships the Twenty→Graphiti
    // adapter without a config row). Pass an empty config; future per-row
