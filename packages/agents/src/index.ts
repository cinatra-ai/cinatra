...
export {
  ...
  // cinatra#3007 — the UNRECORDABLE-hold sentinel and its cap. The recovery is
  // queued where the sentinel is RAISED, not where it is caught, so every path
  // that reaches an unrecordable hold converges the same way. The dispatcher
  // re-enters that same delivery in place only when the raise could not queue
  // it; at the cap nothing re-delivers, and the run is never landed terminal.
