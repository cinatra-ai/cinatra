...
// Built-in assistant-agent registration (cinatra#1037 P1.3) lives in a thin
// vertical slice (the file-size ratchet keeps store.ts from growing further).
// Re-exported here so existing `./store` importers are unchanged.
export {
  BUILT_IN_CINATRA_ASSISTANT_TEMPLATE_ID,
  BUILT_IN_CINATRA_ASSISTANT_PACKAGE_NAME,
...
} from "./builtin-assistant-template";
...
export async function transitionRunStatus(
  ...
  meta?: {
    ...
    dispatch?: AgentRunDispatchFields;
    /** cinatra#1893 (epic #1883 A5): when present, the terminal status CAS, the
     *  final-output snapshot (stepResults/completedAt), AND the unbound-output
     *  derivation-outbox INSERT commit in ONE transaction. Legal ONLY for
     *  `to === "completed"` (asserted below). Absent for every other caller — the
     *  historical two-write path is unchanged. */
  ...
  // cinatra#1893: the transactional terminal-write branch. CAS + snapshot +
  // outbox INSERT are one atomic unit so the derivation candidate is captured
  // iff (and exactly when) this drive wins the terminal transition — the
  // idempotency guarantee AC1 rests on. Only the WayFlow terminal-success path
  // opts in; `to` is asserted terminal-`completed` so the branch never absorbs a
  // dispatch/HITL edge.
  if (meta?.derivationOutbox) {
    ...
    const won = await commitTerminalTransitionWithOutbox(
      runId,
      from,
      to,
      meta,
      meta.derivationOutbox,
    );
    if (!won) {
      throw new RunTransitionError({ code: "stale_from_status", runId, from, to });
    }
    // Side-effects fire AFTER the commit (identical posture to the two-write
    // path): the Redis TTL sweep is best-effort, and the run-wait notify runs
    // even if the TTL schedule throws.
    try {
