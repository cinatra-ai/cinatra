...
import { MAX_REPORTED_TEARDOWN_RUN_IDS } from "./store-force-delete-runs";
import {
 ...
 * Exported type-only for the ./store-template-versions slice (cinatra#2653).
 */
export type AgentTemplateWriteExecutor =
  | typeof db
...
// ---------------------------------------------------------------------------
// Version create / read / bump-classification / changelog — extracted vertical
// slice (./store-template-versions; file-size ratchet, cinatra#2653 fix round:
// the executor-threaded publish cores pushed store.ts over its one-way
// ceiling). Re-exported so every existing `from "./store"` consumer is
// untouched.
// ---------------------------------------------------------------------------
