...
import type { AgentRunRecord, AgentTemplateRecord, CreateAgentRunInput } from "./store";
// The run-start path below (see "THE RUN-START PATH THAT CONSULTS THE HOLD")
// creates, CASes and enqueues the run whose hold this module decides.
import {
  createAgentRun,
  readAgentRunById,
  transitionRunStatus,
...
// ---------------------------------------------------------------------------
//
// Placed in THIS module rather than a new one, for two reasons that agree.
// The decision above is the whole reason the sequence below has the shape it
// has, and reading them together is what makes the ordering self-evident. And
// the locked dev-perf routes are budget-pinned on their reachable first-party
// module count: a new file on this graph would raise four ceilings to hold two
// functions that belong beside the decision they call.
//
