...
/** The ceiling on the composed `agent_runs.error` for a
 ...
 *
 * THE ROW IS THE RECORD; THE FLOOR IS THE READING (cinatra#3033, resolved on the
 * forward merge). Two true things collided here and both are kept, each where it
 * belongs.
 *
 * The ratified drawing's sanitized line — "a sanitized, telemetry-safe one-line
 * diagnostic (package - slot - reason, never a raw error or manifest value)" —
 * is a rule about what a SURFACE draws: "it renders the floor ... so the surface
 * never shows an empty panel where a target should be". It is not a rule about
 * what the run row stores. cinatra#3208's honesty rule is: the stored outcome
 * must name the declaration that was actually resolved, retired identifiers and
 * all, or the record cannot be read back against the version the run executed.
 *
 * So the row keeps the honest sentence, and `run-failure-floor.ts` reduces it
 * for every reader: `runFailureFloorForDisplay` already parses exactly this
 * shape ("outputId [extension]: raw" after the headline), keeps only the output
 * id and the package, and DISCARDS the raw reason — so no producer text reaches
 * a surface either way. Composing the floor here instead would have thrown the
 * identifiers away at the only place they exist, and bought the surface nothing
