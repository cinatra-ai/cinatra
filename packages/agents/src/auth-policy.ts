...
/**
 * Stamp a determinate INSTALL SCOPE on a template about to be written
 * (cinatra#2485 C). That scope is the only thing that authorizes a run, so a
 * template must never be persisted without one.
 *
 * A template anchored to an organization but given no narrower owner anchor IS
 * an organization-scoped template; applying that default at WRITE time is the
 * same rule the canonical install row already uses
 * (`install-package-with-dependencies`: `orgId ? "organization" : …`). Doing it
 * here rather than in the evaluator keeps the read path strictly fail-closed —
 * a legacy row that genuinely carries no anchor still denies, and nothing is
 * guessed at authorization time.
 *
 * This NARROWS: such a template used to be returned (and runnable) for any
 * authenticated actor in any org; now it is runnable by its own org. A template
 * created with NO org anchor at all (a registry fork, say) stays scope-less and
 * is refused at run start — the correct outcome for a row with no tenancy.
 */
export function withDeterminateInstallScope<
  T extends {
    orgId?: string;
