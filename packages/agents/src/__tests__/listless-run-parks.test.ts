...
describe("the drawn surface refuses to continue a list-picking step with no list", () => {
  ...
  it("refuses an answer that CLEARS a list the step was holding", () => {
    // The stored value is a fallback for a submission that says nothing about
    // the list, never for one that names the field and empties it: the emptied
    // answer is the one the seams dispatch, and it names no list.
    expect(
      gateAnswerIncompleteReason(
    ...
    // A submission that says nothing about the list still falls back.
    expect(
      gateAnswerIncompleteReason(
        OUTREACH_LIST_PICKER,
        { note: "unchanged" },
...
describe("the resume seam refuses an answer that names no list", () => {
  ...
  it("refuses a resume whose answer CLEARS the list the gate was holding", () => {
    // The hand-built payload road (the run-resume seam) and a re-submitted form
    // both reach here; the gate's stored list must not stand in for a list the
    // answer itself emptied, because the stored list is not what gets dispatched.
    const gate = { xRenderer: OUTREACH_LIST_PICKER, currentValues: { listId: "lst_old" } };
    expect(
      resumeAnswerIncompleteReason(
        { approved: true, approvalNote: JSON.stringify({ type: "list", listId: "" }) },
        gate,
      ),
    ).toBe(LIST_ANSWER_NAMES_NO_LIST);
    expect(
      resumeAnswerIncompleteReason(
        { userResponse: JSON.stringify({ type: "list", listId: "  " }) },
        gate,
      ),
    ).toBe(LIST_ANSWER_NAMES_NO_LIST);
    // An answer that says nothing about the list still leans on the gate's own.
    expect(
      resumeAnswerIncompleteReason(
        { userResponse: JSON.stringify({ approved: true }) },
        gate,
      ),
    ).toBeNull();
    // And an answer that names one resumes, as before.
    expect(
      resumeAnswerIncompleteReason(
        { userResponse: JSON.stringify({ type: "list", listId: "lst_new" }) },
        gate,
      ),
    ).toBeNull();
  });
