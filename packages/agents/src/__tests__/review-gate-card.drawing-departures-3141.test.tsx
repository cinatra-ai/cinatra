...
vi.mock("../run-window-actions", () => ({
  ...
  // THE SEND'S OWN ANSWER (cinatra#2934, lifecycle-b W5c): the turn reports what
  // it did — the entries it stored, any fill it placed, and whether it actually
  // pressed a control. A mock that omits them lies about the contract.
...
describe("#3141 item 1 — the conversational prompt window is part of the gate", () => {
  ...
  it("a landed change request KEEPS the exchange on screen — the card does not re-resolve it away", async () => {
    ...
    // THE WINDOW FILES NOTHING ITSELF (cinatra#2934, lifecycle-b W5c). What is
    // typed reaches the assistant, and a request for changes is filed through
    // the card's OWN Comment control under the person's own grant — so the
    // window never composes a disposition, and a send that pressed nothing
    // (`acted: false`) leaves the gate exactly where it was.
