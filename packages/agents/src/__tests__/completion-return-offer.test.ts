...
/**
 * EVERY WRITER OF THIS CONTRACT CUTS AN ADDRESS THE SAME WAY (convergence round,
 * cinatra#3358). The produced-id writer already separated the fragment and cut
 * the query at its FIRST question mark; the two return writers still used a
 * naive `split("?", 2)`, so a fragment swallowed the contract and a value of its
 * own carrying a question mark truncated the address. All three are pinned here
 * on the same three addresses.
 */
describe("the contract survives a fragment and a question mark in a value", () => {
  const ret = { onComplete: "list-picker", returnRunId: "run-parked" } as const;

  it("keeps the fragment at the end and the keys in the SEARCH string", () => {
    const href = withCompletionReturn("/w/acme/agents/v/p/run-parked#review", ret);
    const url = new URL(href, "https://example.test");
    expect(url.hash).toBe("#review");
    expect(url.pathname).toBe("/w/acme/agents/v/p/run-parked");
    expect(url.searchParams.get("onComplete")).toBe("list-picker");
    expect(url.searchParams.get("onCompleteRunId")).toBe("run-parked");
  });

  it("never folds a fragment into an existing value", () => {
    const url = new URL(
      withCompletionReturn("/w/acme/agents/v/p/run-parked?tab=run#review", ret),
      "https://example.test",
    );
  ...
  it("keeps a value that carries its own question mark whole", () => {
    const url = new URL(
      withCompletionReturn("/w/acme/agents/v/p/run-parked?next=a?b", ret),
      "https://example.test",
    );
    expect(url.searchParams.get("next")).toBe("a?b");
    expect(url.searchParams.get("onCompleteRunId")).toBe("run-parked");
  ...
  it("holds for the new-run writer too, with and without a run in hand", () => {
    const withRun = new URL(
      newRunHrefWithCompletionReturn(
        "/workspace/agents/v/p/new?tab=run#review",
        "list-picker",
        "run-1",
      ),
      "https://example.test",
    );
    expect(withRun.hash).toBe("#review");
    expect(withRun.searchParams.get("tab")).toBe("run");
    expect(withRun.searchParams.get("onCompleteRunId")).toBe("run-1");

    const bare = new URL(
      newRunHrefWithCompletionReturn(
        "/workspace/agents/v/p/new?next=a?b#review",
        "list-picker",
        null,
      ),
      "https://example.test",
    );
    expect(bare.hash).toBe("#review");
    expect(bare.searchParams.get("next")).toBe("a?b");
    expect(bare.searchParams.get("onComplete")).toBe("list-picker");
    expect(bare.searchParams.has("onCompleteRunId")).toBe(false);
  });

  it("holds for the produced-id writer, unchanged", () => {
    const url = new URL(
      withCompletionProduced("/w/acme/agents/v/p/run-parked?next=a?b#review", "lst_3"),
      "https://example.test",
    );
    expect(url.hash).toBe("#review");
    expect(url.searchParams.get("next")).toBe("a?b");
    expect(url.searchParams.get(COMPLETION_PRODUCED_PARAM)).toBe("lst_3");
  });
});
