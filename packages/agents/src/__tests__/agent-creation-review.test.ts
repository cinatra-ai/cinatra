...
describe("agent_creation_review — dispatch contract", () => {
  it("WI-5 BLOCKER 1 PARITY: pin INACTIVE → each lane uses runDeterministicLlmTask with provider:openai, model:gpt-5", async () => {
  ...
  it("WI-5 BLOCKER 4 REGRESSION: preflight ok:false short-circuits before any LLM call (deterministic blocker)", async () => {
  ...
  it("WI-5 BLOCKER B REGRESSION: AgentCreationDispatchAbortError → deterministic blocker (not downgraded warning)", async () => {
