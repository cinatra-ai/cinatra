/**
 * eng#378 TIER-1 — buildAgentTemplateInstallSeed.
 *
 ...
 * tarball that ships `cinatra/oas.json` and NO root `agent.json`. Pre-eng#378
...
describe("buildAgentTemplateInstallSeed (eng#378 TIER-1: OAS-only direct seed)", () => {
  it("seeds the agent_templates row from cinatra/oas.json with NO root agent.json (the eng#378 case)", async () => {
  ...
  it("snapshot includes outputSchema + lgGraph* + sourceNl (rollback/diff-stable per §B2.3)", async () => {
    const dir = await stageOasOnlyPackage();
    const seed = await buildAgentTemplateInstallSeed({
      extractedTempDir: dir,
      packageName: PACKAGE_NAME,
