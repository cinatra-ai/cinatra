...
describe("the swap happens on its own while the stream stays mute", () => {
  ...
  it.each(SURFACES)(
    ...
    async (_name, surface, wrap) => {
      ...
      // is the reading under test and it is the same on both. In a conversation
      // the pause is the run progress panel, under the cinatra#3484 contract
      // that the thread's own screen card asks the question.
      await waitFor(() => {
