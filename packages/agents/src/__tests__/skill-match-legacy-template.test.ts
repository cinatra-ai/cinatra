...
describe("agent_delete — skill-match cleanup legacy-template behavior", () => {
  ...
  it("B1-01: cleanupForAgent is NOT called when template.packageName is null", async () => {
  ...
  it("B1-02: structured warning is emitted with the legacy-template event marker", async () => {
    const handlers = await setupHandlers();
    await configureAuthPasses();
  ...
  it("B1-03: cleanupForAgent IS called with packageName when packageName is set", async () => {
    const handlers = await setupHandlers();
    await configureAuthPasses();
  ...
  it("B1-04: empty-string packageName is treated as missing (falsy)", async () => {
    const handlers = await setupHandlers();
    await configureAuthPasses();
