...
// terminal write. This suite is about the transcript receipt, drives the tail
// with no database, and would otherwise exercise the hold's FAIL-CLOSED branch
// (an unreachable store cannot prove the run owes no review, so no terminal
// status is written). Mocked inert here — "nothing holds this run" — so the
// tail under test behaves exactly as it did. The hold itself is proven in
