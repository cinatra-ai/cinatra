...
// ---------------------------------------------------------------------------
// Export-side OAS repair (cinatra#2645)
// ---------------------------------------------------------------------------
//
// agent_export ships the canonical on-disk `cinatra/oas.json` byte-for-byte,
// and that file is not always written by the validated `agent_source_write`
// path. Wayflow-style writers (the Oracle agent-spec exporter and documents
// that entered disk through proposal materialization or dev mounts) serialize
// every absent optional field as an explicit `null` and omit
// `metadata.cinatra.type` entirely. This compiler rejects exactly those two
// artifacts (`flowSchema.metadata.cinatra.type` is a REQUIRED enum; the
// connection/port descriptions are `z.string().optional()` — string or
// ABSENT, never null), so export -> re-import was dead for any such agent.
//
// The repair lives HERE, beside the schemas it repairs to, so the exporter
// and the compiler can never diverge — and stays out of the locked
// route-graph module counts (this module is already reachable on every
// locked route, and mcp/handlers.ts sits one line under its own
// file-size-ratchet ceiling). The compiler itself intentionally stays
// strict: the exporter emitting `null` for an optional string is the defect
// (#2645). Repairs are deterministic and idempotent; a document that needs
// no repair is reported `changed: false` so the export keeps shipping the
// original bytes byte-for-byte (the #130 contract).
 ...
 * (1) a Flow document missing (or carrying a non-enum)
 * `metadata.cinatra.type` gets the type derived from its component_type
 * (Flow -> "flow"); (2) every `description: null` is removed recursively.
 * A `null` description carries no information — the compiler treats an
 * absent description as the default everywhere, and the one schema that
 * tolerates `null` (`parallelFlowNodeSchema.description`) treats it
 * identically to absent. Pure and deterministic; the input is never mutated.
 */
export function normalizeOasDocumentForExport(
  ...
  // is absent, null, or not a valid enum value. An existing valid enum value
...
/**
 * String-level wrapper for the export handler: parse, repair, and
 * re-serialize (2-space, trailing newline — the on-disk writer format) ONLY
 * when a repair fired; return the original string otherwise, preserving
 * byte-for-byte fidelity for valid documents. A non-JSON input is returned
 * unchanged — the importer's own JSON parse rejects it with an explicit
 * error.
 */
export function normalizeOasJsonForExport(raw: string): string {
  try {
    const normalized = normalizeOasDocumentForExport(JSON.parse(raw) as Record<string, unknown>);
    return normalized.changed ? JSON.stringify(normalized.doc, null, 2) + "\n" : raw;
  } catch {
    return raw;
  }
}
