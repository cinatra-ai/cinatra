...
export const AGENT_BUILDER_TOOL_META: Record<string, ToolMeta> = {
  ...
  "email_outreach_initial_drafts_update": {
    description:
      "Run-scoped persist of the operator's reviewed per-recipient draft edits (subject/body) onto THIS run's own draft-bundle object — the write the re-entrant drafts / follow-ups review gate's post-resume apply node dispatches. The run, its declaring agent package (restricted to packages that serve an email-drafts-review mid-run gate, resolved from the manifest), and the actor all come from the run-bound invocation context; a caller runId/campaign is ignored. Only callable from inside such a run (the deterministic pre-interrupt seam or an agent-run OBO frame); a bare chat/session call fails closed. Returns { ok, runId, agentPackageName, matched, updated } (ok:false + reason on a genuine persist gap).",
    inputSchema: z.object({
      drafts: z
        .array(
          z.object({
