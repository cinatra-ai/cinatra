...
export const AGENT_BUILDER_TOOL_META: Record<string, ToolMeta> = {
  ...
  "agent_run": {
    ...
    inputSchema: z.object({
      ...
      packageName: z.string().optional().describe("Package name of the agent to run (e.g. '@cinatra-ai/media-feed-lister-agent'). Resolved against `agent_templates.package_name` with current-vendor → `@cinatra-ai/<slug>` alias fallback. Mutually exclusive with `templateId`."),
  ...
  "agent_list": {
    ...
    inputSchema: z.object({
      ...
      packageName: z.string().optional().describe("Exact package name filter (e.g. '@cinatra-ai/email-recipient-selection-agent'). When provided, only templates with this exact packageName are returned."),
...
"agent_creation_review": {
  ...
  "agent_creation_request_propose": {
    ...
    inputSchema: z
      .object({
        ...
        packageName: z.string().describe("@scope/name. Must NOT collide with an existing agent_template (ACAW-COLLISION-01)."),
  ...
  "agent_creation_request_decide": {
    description:
      ...
      "under the approving admin's actor frame (private-scoped, ACAW-COLLISION-01 hard-rejects on " +
