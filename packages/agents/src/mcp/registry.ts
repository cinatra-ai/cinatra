  ...
  for (const [name, handler] of Object.entries(handlers)) {
    ...
    server.registerTool(
      ...
      (async (input: unknown) => {
        ...
        // META-01/02/03 — render hints for HITL-adjacent tools only.
        // P8 guard: handlers return { error: ... } on failure; check shape before casting.
        // P2 guard: this _meta lives on the CALLBACK RETURN, not on the tool definition above.
