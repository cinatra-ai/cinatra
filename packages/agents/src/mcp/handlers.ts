...
// CF-02: auth gate runs in caller BEFORE this import is used.
// CF-03: resolvePublishDestination must be called before publishAgentPackageFromGitDir.
...
// ---------------------------------------------------------------------------
// AUTH-03: resolve the active org id from the session at request
...
// ---------------------------------------------------------------------------
// AUTH-03: resolve isPlatformAdmin from the session at request
...
async function handleAgentBuilderList(
  ...
  try {
    ...
    // AUTH-03: scope reads to session.activeOrganizationId.
    ...
    // AUTH-03: collapse AuthzError to spec'd surface error.
...
async function handleAgentBuilderUpdate(
  ...
  try {
    // G1 — set packageName only when null; reject if already set.
    if (hasPackageName) {
...
async function handleAgentBuilderRunList(
  ...
  try {
    ...
    if (templateId && !templateIds) {
      ...
      // Use unfiltered DB total (WR-04) so pagination math stays consistent.
      // total reflects post-filter row count (matches the
      // empty-list contract asserted by mcp-run-read-policy Test 4). When all
      // rows are denied this returns { items: [], total: 0 } plus per-row
      // denial audits. Trade-off vs pagination math is now based on
  ...
  try {
    ...
    if (
      ...
      await writeHitlPrompt({
        ...
        stepKey: run.a2aTaskId,                        // bare task.id, no `wayflow-` prefix (Pattern S2)
        message: trimmedNote,
        submittedValues,
...
async function handleAgentBuilderGitPublish(
  ...
  // CF-02 requires the caller to run an auth gate; the MCP registration loop
  ...
  // SPDX license detection gate (LD-06).
  ...
  // call resolvePublishDestination(destination) after auth gate (CF-02).
  // CF-03: resolvePublishDestination must appear BEFORE publishAgentPackageFromGitDir.
  //
  ...
  // ahead of resolvePublishDestination so the CF-03 resolver→publish proximity
  ...
  try {
    ...
    if (result.published && result.packageName && result.packageVersion) {
      ...
      // persist origin coordinates after successful publish (LD-03).
      // LD-02: tokens MUST NOT appear in origin; only opaque destinationId written.
...
async function handleAgentBuilderRegistryPublish(
  ...
  // route through resolvePublishDestination (CF-02, CF-03).
