...
//
// The four canonical declarative kinds. Source-authoring is declarative-first:
// /chat authors ARTIFACT/SKILL packages in v1; code-bearing CONNECTOR
...
// ---------------------------------------------------------------------------
const CANONICAL_EXTENSION_KINDS = ["agent", "connector", "artifact", "skill"] as const;
 ...
 * pipeline passes the historical "agent"; the artifact/skill source
...
async function handleAgentBuilderGitWriteFiles(
  request: PrimitiveRequest<{
    ...
    // the artifact/skill source tools pass their own canonical kind so
  ...
  if (requestedKind !== undefined) {
    if (!isCanonicalExtensionKind(requestedKind)) {
      return {
        error: `Unsupported kind "${String(requestedKind).slice(0, 40)}". Declarative authoring supports: agent, artifact, skill.`,
      };
    }
    if (requestedKind === "connector") {
      return {
        error:
          "Connector authoring is not available — code-bearing connector packages are gated on SDK-P0 (#162). Declarative kinds only: agent, artifact, skill.",
      };
    }
  ...
  // while the artifact/skill source tools pass their own kind. Same
...
// boundary (same gate as agent_source_*) and are denied for
...
// NOT admin-gated (parity with agent_source_validate).
// ===========================================================================
...
async function validateArtifactPackageOnDisk(
  ...
  if (extraneous.length > 0) {
    errors.push(`artifact extensions may only declare cinatra.{kind,apiVersion,artifact,dependencies,roles,displayName,vendor,views,fieldRenderers}; unexpected key(s): ${extraneous.join(", ")}.`);
  }
...
async function handleArtifactSourceWrite(
  ...
  // the agent path) so disk slug, package name, and published scope
...
// Like the artifact compile (and UNLIKE agent_source_compile), there is NO
...
async function handleArtifactSourceCompile(
  ...
  // sibling-file credential scan over the on-disk package (parity with the artifact compile).
...
//
// Pipeline shape mirrors the artifact path:
...
// Mirrors handleArtifactSourcePublish exactly (admin gate → on-disk
...
// over the kind + its on-disk validator + its path resolver. The artifact path
...
async function publishDeclarativePackage(
  ...
  // never publishes (parity with the artifact path).
  ...
  // SPDX license detection gate — parity with agent/artifact publish.
  try {
  ...
  // Public-publish strict-semver guard — parity with agent/artifact publish.
  if (destination === "public") {
    let pkgVersion = "";
    try {
  ...
  // Publish-side destination resolution (parity with the artifact path).
...
async function handleEmailTestDeliveryRunSend(
  request: PrimitiveRequest<{
...
async function handleEmailTestDeliveryParseAction(
