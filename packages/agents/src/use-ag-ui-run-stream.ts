...
export function useAgUiRunStream(
  ...
  useEffect(() => {
    ...
    // WR-01 fix: reset run-scoped state so prior run output never leaks into a new runId.
    ...
    es.onmessage = (ev: MessageEvent<string>) => {
      ...
      switch (event.type) {
        ...
        case "RESUME": {
          // CR-03: INTERRUPT and RESUME do NOT close the stream — the run continues
    ...
    es.onerror = () => {
      // CR-01 fix: do NOT read `status` here — the closure is seeded when the
