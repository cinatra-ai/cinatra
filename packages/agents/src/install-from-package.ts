...
// ---------------------------------------------------------------------------
// agent_source_compile DB write-back
// ---------------------------------------------------------------------------
// Lives HERE, beside the install-path seed builder, and NOT on any route-side
// module: it is a compile/install-side concern and the route graph must not
// grow a module for it (cinatra#3208 route-graph ratchet). After a recompile
// has rewritten agent.json on disk, re-project the compiled result onto the
// agent_templates row so subsequent runs read the current source WITHOUT
// recompiling. The version-pairing rule the block encodes is documented once,
// below.
