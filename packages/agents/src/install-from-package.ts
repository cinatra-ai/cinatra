...
// eng#378 TIER-1: seed the agent_templates row DIRECTLY from cinatra/oas.json +
...
async function _installAgentFromPackageImpl(
  ...
  try {
    ...
    // eng#378 TIER-1: derive the agent_templates row seed DIRECTLY from the
...
async function registerDeclaredObjectTypes(opts: {
  ...
  try {
    const compileResult = await compileOasAgentJson({
      ...
      // to eng#378 this passed <tempDir>/agent.json, which does NOT exist for
