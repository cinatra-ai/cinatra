...
// schedules + typed placeholders). Snapshot-on-instantiate (D10): a workflow
...
// Postgres is the single source of truth (D3).
// ---------------------------------------------------------------------------
export const workflow = cinatraSchema.table(
  "workflow",
  {
    id: text("id").primaryKey(),
    // Snapshot provenance — plain text, no FK (D10 immutability/independence).
...
// and first-class failure/retry/cancel/missed-window policy (D18).
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// workflow_dependency — execution edges, separate from schedule.anchor (D7).
...
// ---------------------------------------------------------------------------
// workflow_approval — workflow-native, human-only approval (D11). Own
