...
Skills-unaffected: agent_run — security/authz-internal changes only: A2A service-identity actor resolution removed from the MCP run path (sec hardening); the run-side SoD self-approval guard (#585) and its operator-vendor `connector_config.agent_run.allowSelfApproval` policy were later REMOVED entirely (cinatra#2485 item A) — install/later-set scope is the run-authorization gate, and the HITL approve/reject server actions authorize the actor against the run (`run.execute` + `run.approveHitl`) instead of demanding a platform-admin session (item B). The primitive name and its documented `agent_run { packageName, inputParams }` shape are unchanged; no public SKILL.md dispatch instruction drifts.
