...
**The two things the withdrawn round did are not done here, and what stands in
each place is stated with its limit rather than promised.** The turn names the
...
and it is an argument rather than a proof.** `resolveConfiguredLlmRuntime` — the
...
PLAN> On the run page the same row sits at the **trigger position**, the top entry on the step rail, ahead of the work steps it would authorize.
DRAWING> design-run-surface-rail-and-gate.png
DRAWING-CHECK> requires: the same decided run page in the dark palette — the settled `Recommendation` entry still on the rail with its check, the run's own panel in the run detail beside it, the settled chips with nothing left to press / shows, MEASURED: the same run, the same shutter contract (1440x1700 CSS px at deviceScaleFactor 2, `framing: "window"`), and a record whose `runSurface` block is field-for-field the light cell's — `railRowLabels: ["Recommendation", "Step 1Review pending policycore-defaultawaiting review orche"]` (the second entry is the rail column's own text run, truncated by the readout at 60 characters), `railStepSettled: "true"`, `railStepSelected: "false"`, `railStepIndicatorHasCheckGlyph: true`, both instrumented columns at 1, `chipRowInDetailColumn: true`, three chips with empty `actions`, all three `[data-skill-action]` counts **0** inside the card root. `themeClass` carries `dark`. / and READ OFF THE PIXELS: `✓ Recommendation`, `✓ Step 1`, `▷ Review pending policy` on the left, all legible on the dark ground; the `Agentic Run Progress` card on the right with its `completed` pill; the same three chips reading `ADJUSTED` / `CONFIRMED` / `CONFIRMED`; `Run complete` and `Start new run` below them / verdict: PASS, with the same two stated readings as the light cell

## What these eight cells now show, and the one reading that stands

**The two things the withdrawn round did are not done here — and what that does
and does not settle is set out immediately below.** The deterministic pre-router did not dispatch this run —
the turn carries no package token and both its counters read 0 on every cell — so
the run was started through the platform's own `agent_run` tool. No step of this
sequence clears the sealed provider row, and the row is read back present before
the agent's step and again after it; two point reads bracket the call rather than
establishing uninterrupted presence. The person decides each chip on the shipped
card, and the run executes and writes a real artifact.

**What is bounded rather than claimed** is which runtime served each model call.
For the agent's step the resolver's ordering plus the two provider reads make the
scripted runtime very unlikely, and the residual — an adapter that failed to
resolve at that instant — is not measured here. For the chat turn the seam checks
the scripted flag first, and the read that speaks to it is taken one hop above the
listening process: presence would be proof, absence there is consistent rather
than conclusive. Neither residual is closed by a field in these records, and
neither is written as if it were.
