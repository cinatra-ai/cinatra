...
**This round performs NEITHER OF THE TWO ACTIONS the withdrawn round performed —
a statement about what the driver does, not about which runtime answered — and
what stands in each place is stated with its limit rather than promised.** The turn names the
...
either, and its counters read 0 on every cell. That the run was therefore started
by a MODEL calling `agent_run` is an ARCHITECTURAL INFERENCE from that absence
plus the run's own chat-launch carrier — not a measurement: no committed field
records who invoked the tool, and the `/api/mcp` counter is expressly
unattributed. The limits are set out below.
