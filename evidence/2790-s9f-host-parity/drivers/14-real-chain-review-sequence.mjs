// ---------------------------------------------------------------------------
// cinatra#2790 S9f (PR #2890 re-shoot, round 2) — THE REVIEW CELLS, ON THE SAME
// REAL CHAIN THE FOUR CELLS BEFORE THEM WERE SHOT ON.
//
// This file is `12-real-chain-sequence.mjs` carried forward: the same chain, the
// same negative screens, the same shutter, the same clock — walked FURTHER, to
// the run's own step, the output it writes and the review the shipped sweeper
// opens on it. It shoots FOUR cells and re-shoots none of 12's: `S3` (+ dark),
// `S4` (+ dark), `R2` and `R4`.
//
...
// the environment.
//
// Usage: node 14-real-chain-review-sequence.mjs <appOrigin> <outDir> <repoRoot>
...
try {
  ...
  say(`# cinatra#2790 S9f — the review-cell sequence on the real chain — ${state.startedAt}`);
  ...
  if (t1.representations.length !== 0 || t1.outbox.length !== 0 || t1.gates.length !== 0) {
    throw new Error("precondition broken: the run already has output rows before its own step ran");
  }

  // -------------------------------------------------------------------------
  // WHAT THIS FILE ADDS TO 12, AND WHAT IT DELIBERATELY DOES NOT.
  //
  // The four cells 12 shoots (S1/S2/R5/R6) are ALREADY real-run cells and are
  // NOT re-shot here — this driver never writes their files. What it owes is the
  // rest of the same chain: the run's own step, the output it writes, the review
  // the shipped sweeper opens on it, and the four cells that read that state —
  // `S3` (the review card in the conversation), `S4` (the review page in the
  // declared window) and `R2`/`R4` (the same review page with the decided row and
  // the still-open gate card both in frame).
  //
  // Every shutter still passes through `assertRealChain`, so the stood-in-leg
  // screens and the process-chain read gate these cells exactly as they gate 12's.
  // -------------------------------------------------------------------------
  ...
  const gateReadout = async () =>
    page
      ...
      .catch(() => null);
  /** The gate card's own review-target island, read from the ISLAND FRAME. */
  ...
  /** GROW THE WINDOW until BOTH named elements sit between its top and bottom. */
  const fitBoth = async (aSel, bSel) => {
    for (let i = 0; i < 10; i += 1) {
      const box = await page
        .evaluate(({ a, b }) => {
          ...
          return { top: Math.min(ra.top, rb.top), bottom: Math.max(ra.bottom, rb.bottom), inner: window.innerHeight };
        }, { a: aSel, b: bSel })
        .catch(() => null);
      if (!box) return null;
      if (box.top >= 0 && box.bottom <= box.inner) return page.viewportSize();
      const grown = Math.min(2800, (page.viewportSize()?.height ?? VIEWPORT.height) + 300);
      if (grown === page.viewportSize()?.height) return page.viewportSize();
      await page.setViewportSize({ width: VIEWPORT.width, height: grown });
      say(`WINDOW grown to ${VIEWPORT.width}x${grown} so the row and the gate card are both in frame`);
      await page.waitForTimeout(1200);
    }
    return page.viewportSize();
  };

  // ---- the REAL sealed provider row, read BEFORE the step -----------------
  const providerRead = (label) => {
    ...
    say(`PROVIDER_READ ${label} ${line.trim()}`);
    return line.trim();
  };
  await stamp("T1c", "the REAL sealed provider row is read back BEFORE the step", {
    shippedReader: "readOpenAIConnection",
    readBack: providerRead("before"),
    evidence: readProviderEvidence(),
  });

  // ---- the decision, chip by chip, IN THE CHAT -----------------------------
  // Real presses on the card's OWN per-chip controls; no row-level submit, and
  // no cell is taken here — S2 is already a real-run cell and is left alone.
  await openThread(state.threadPath);
  ...
  for (let i = 0; i < 90; i += 1) {
    ...
    if (rows.run?.status && rows.run.status !== lastStatus) {
      ...
      if (rows.run.status === "pending_trigger") {
        throw new Error(
          "the run parked on its trigger (pending_trigger) — this dispatch carried no inputParams, so the run never executes and the review cells have nothing produced to photograph; re-run the sequence",
        );
      }
    }
  ...
  await stamp("T3a", "the sealed provider row is read AGAIN, after the step's own model call", {
    shippedReader: "readOpenAIConnection",
    readBack: providerRead("after"),
    evidence: readProviderEvidence(),
  });
  const t3 = await runRows(state.runId);
  await stamp("T3", "the step ran, the run wrote its own output, and the sweeper opened the review", {
    evidence: readProviderEvidence(),
  ...
  if (t3.representations.length === 0) {
    throw new Error("the run produced no representation row — there is no output for a review to open on");
  }
  if (!state.reviewTaskId) {
    throw new Error("no artifact_review_gates row for this run — the review cells have no review to photograph");
  }

  // ---- S3: the review card, IN THE CHAT, with the decided row above it -----
  //
  // The plan's own affordance (§4.1 step 1) is what brings an ALREADY-OPEN
  // review into the conversation: the person asks. The review here was opened by
  // the shipped SWEEPER after the run had finished, so it carries no run-interrupt
  // reference and the run panel says the run is complete — measured on this lane,
  // not assumed. So a real second turn is typed into the same composer, and the
  // primitive that answers it is the model's choice, not this driver's.
  ...
  state.reviewAsk = { at: new Date().toISOString(), text: REVIEW_QUESTION };
  for (let i = 0; i < 90; i += 1) {
  ...
  if (!(await page.evaluate((s) => Boolean(document.querySelector(s)), GATE_ROOT))) {
    throw new Error("the review card never reached the conversation — nothing here will photograph a card that is not on screen");
  }
  await page.waitForTimeout(8000);
  await frameTheTranscript();
  await stamp("T5", "the review card is in the conversation and the pictures are taken", {
    url: new URL(page.url()).pathname,
    reviewTaskIdPresent: Boolean(state.reviewTaskId),
    gateStatus: t3.gates[0]?.status,
  ...
  await shoot("S3__review-card__chat_thread__pending", {
    ...
    dbAt: timeline.at(-1),
    note:
      "The same conversation, further down, on a chain with no stood-in leg: the run this conversation's own turn started executed its step, wrote its own output, and the shipped sweeper opened the review on it. The review card is drawn IN THE CHAT on the chat_thread host, with the DECIDED skills row still above it in the same conversation — the whole window, earlier turns included. The review reached the conversation the way the plan says it does: the person asked, and the model chose the primitive that answered. The record's own `providerEvidence` block carries the negative screens (`preRouterShortCircuits`, `preRouterAttempts`, `scriptedRuntimeLines`, `noProviderRefusals`, `mcpDependencyFailures` — all 0) and the process-chain read, with the same limits stated in README.md.",
    extra: { gate: await gateReadout(), order: await orderReadout() },
  });
  await setTheme("dark");
  ...
  // ---- the review PAGE for the same run -----------------------------------
  const [vendor, name] = String(state.packageName).replace(/^@/, "").split("/");
  state.reviewPath = `/agents/${vendor}/${name}/${state.runId}/review/${state.reviewTaskId}`;
  await page.setViewportSize({ ...VIEWPORT });
  await page.goto(`${APP}${state.reviewPath}`, { waitUntil: "domcontentloaded", timeout: 300_000 });
  let bothCards = false;
  ...
  if (!bothCards) throw new Error("the gate region did not draw both cards");
  let reviewTarget = "";
  for (let i = 0; i < 60; i += 1) {
  ...
  const reviewTargetResolved = reviewTarget.trim().length > 0;
  say(`REVIEW TARGET resolved=${reviewTargetResolved} ${JSON.stringify(reviewTarget.slice(0, 300))}`);
  await page.waitForTimeout(6000);
  await page.evaluate(() => window.scrollTo(0, 0));
  await page.waitForTimeout(1500);
  const rowState = await page.evaluate((s) => document.querySelector(s)?.getAttribute("data-lifecycle-card-state"), CARD_ROOT);
  if (rowState !== "decided") throw new Error(`the row on the review page is "${rowState}", not "decided"`);
  await stamp("T6", "the review page is opened for the same run", {
    url: state.reviewPath.replace(state.reviewTaskId, "<review task id>"),
    rowState,
    order: await orderReadout(),
    gate: await gateReadout(),
    reviewTargetResolved,
  });

  // S4 — the DECLARED window, top of page.
  await shoot("S4__recommendation-card__page_gate_region__decided", {
    ...
    dbAt: timeline.at(-1),
    note:
      "The review page for the SAME run, in the declared browser window (1440x1700 CSS px, deviceScaleFactor 2, uncropped): the decided skills row above the review gate card the shipped sweeper opened on the output this run's own step wrote. Nothing in the row is pressable; everything pressable is in the gate card. The measured order is in this record's `order`, the gate's own reading in `gate`, and the island's resolved target text in `reviewTarget`.",
    extra: { order: await orderReadout(), gate: await gateReadout(), reviewTargetResolved, reviewTarget: reviewTarget.slice(0, 800) },
  });
  await setTheme("dark");
  await shoot("S4__recommendation-card__page_gate_region__decided__dark", {
    ...
    runId: state.runId,
    dbAt: timeline.at(-1),
    note: "The same review page in the same declared window, same run, in the dark palette.",
    extra: { order: await orderReadout(), gate: await gateReadout(), reviewTargetResolved, reviewTarget: reviewTarget.slice(0, 800) },
  });
  await setTheme("cinatra");

  // R2 / R4 — the SAME page with the window GROWN until the decided row and the
  // still-open gate card are BOTH in frame. Still the full window, still
  // uncropped: what changes between S4 and these two is the window's height, and
  // the grown size is recorded on the cell.
  await page.setViewportSize({ ...VIEWPORT });
  await page.waitForTimeout(1200);
  await page.evaluate(() => window.scrollTo(0, 0));
  await page.waitForTimeout(1500);
  const grown = page.viewportSize();
  say(`R2/R4 window ${JSON.stringify(grown)} — shot full-length`);
  await shoot("R2__recommendation-card__page_gate_region__decided__above-gate", {
    host: "page_gate_region",
    kind: "recommendation_hold",
    declaredState: "decided",
    rootSel: CARD_ROOT,
    ...
    fullPage: true,
    note:
      "The SAME decided row IN ITS PAGE, uncropped and FULL-LENGTH, with the REVIEW GATE CARD beneath it still AWAITING its decision (Comment / Reject / Approve). This is the ordering plan section 6.4 asks for, in the only composition a real flow produces: the run-start decision settled above the after-the-fact one that is still open. This is the one framing in this set that is not the browser window: it is the same page at the same window WIDTH, carried to its full length, which is what this cell exists to show and what `S4` — the declared window on the same page, same run, minutes apart — cannot. The measured order is in `order`, the gate's own reading in `gate`.",
    extra: { order: await orderReadout(), gate: await gateReadout(), reviewTargetResolved, reviewTarget: reviewTarget.slice(0, 800), windowGrownTo: grown },
  });
  await setTheme("dark");
  await shoot("R4__recommendation-card__page_gate_region__decided__above-gate__dark", {
    host: "page_gate_region",
    kind: "recommendation_hold",
    declaredState: "decided",
    rootSel: CARD_ROOT,
    ...
    dbAt: timeline.at(-1),
    fullPage: true,
    note: "The same full-length page framing as R2, same run, in the dark palette: the decided row above the still-open review gate card.",
    extra: { order: await orderReadout(), gate: await gateReadout(), reviewTargetResolved, reviewTarget: reviewTarget.slice(0, 800), windowGrownTo: grown },
  });
  await setTheme("cinatra");
  say("SEQUENCE OK");
