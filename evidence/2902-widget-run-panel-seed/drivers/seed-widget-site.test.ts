// cinatra#2902 run-panel capture, step 1 — register a widget INSTANCE and its
// connect-site so the `site_widget` host can be driven without a WordPress
// container.
//
// WHY THIS FILE EXISTS AT ALL. Without an instance row and a connect-site the
// embed frame sits at `data-phase="signin"` and its hosted-PKCE popup dies —
// not because the ceremony is unavailable on a lane, but because the widget was
// never provisioned. With the two rows below written, the popup completes and
// the frame mints its own `cwu_`, which is the credential this whole round is
// about.
//
...
//
// Usage: pnpm vitest run --config evidence/2902-widget-run-panel-seed/drivers/seed-widget-site.config.ts
...
it("seeds the widget instance and its connect-site", () => {
  ...
  kept.push({
    ...
    name: "2902 run-panel widget capture site",
    username: "cap2902-capture",
    applicationPassword: "2902 capture placeholder (no WordPress is called)",
  });
