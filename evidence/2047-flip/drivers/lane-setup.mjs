// cinatra#2047 activation-flip lane — first-admin signup + org + ids for the walk.
// Adapted from evidence/2047-d8/drivers/lane-setup.mjs (same shape, this lane's port/DB).
import { Client } from "pg";
import fs from "node:fs";

const BASE = process.env.WALK_BASE ?? "http://localhost:3167";
const EMAIL = "lane-2047-flip@local.test";
...
console.log("sign-up", (await post("/api/auth/sign-up/email", { email: EMAIL, password: PASSWORD, name: "Lane 2047 Flip" })).status);
...
if (!orgId) {
  const cr = await post("/api/auth/organization/create", { name: "Lane 2047 Flip Org", slug: "lane-2047-flip-org" });
...
const tpl = await client.query(
  `SELECT id, package_name FROM cinatra.agent_templates WHERE package_name = '@cinatra-ai/blog-draft-writer-agent' LIMIT 1`,
);
await client.end();
...
const out = {
  ...
  templatePkg: tpl.rows[0]?.package_name ?? null,
  cookie: cookieHeader(),
};
