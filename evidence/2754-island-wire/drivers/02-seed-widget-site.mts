// cinatra#2754 island capture, step 2 - register a widget INSTANCE and its connect-site
...
const WIDGET_ORIGIN = process.argv[2] || "http://127.0.0.1:5591";
const INSTANCE_ID = process.argv[3] || "isl2754-local-site";
const CLIENT = "wordpress";

const org = process.env.ISL_ORG_ID;
const admin = process.env.ISL_ADMIN_USER_ID;
if (!org || !admin) throw new Error("set ISL_ORG_ID and ISL_ADMIN_USER_ID");
...
kept.push({
  ...
  name: "2754 island capture site",
  username: "isl2754-capture",
  applicationPassword: "2754 capture placeholder (no WordPress is called)",
});
