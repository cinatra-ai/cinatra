/**
 * cinatra#2754 capture lane — produce a REAL review gate whose TARGET is a REAL
 * artifact, so the island the card frames has something to paint.
 *
 ...
 *
 * and the ref the card carries is minted by the shipped codec
 * (`encodeLifecycleGateRef`) against THAT gate. The evidence value of the round
 * depends on the target being real: an island that paints an empty floor proves
 * nothing about a credential, because an unauthenticated island paints an empty
 * document too. The two must be distinguishable on screen, so the gate is given
 * a target with visible content.
 *
