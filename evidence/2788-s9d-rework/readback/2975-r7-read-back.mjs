// THE ROUND 7 READBACK (cinatra#2970, PR #2975).
//
...
const pick = (re, cap = 12) => log.split("\n").filter((l) => re.test(l)).slice(0, cap)
  .map((l) => l.replace(/https:\/\/[^\s/]+/g, "<the lane's public origin>"))
  .join("\n");
writeFileSync(resolve(TXT), [
  "# Round 7 runtime evidence (cinatra#2970, PR #2975).",
  ...
  "",
  "## grep -E 'explicit-dispatch pre-router HARD short-circuit'  (the two runs that carry NO cell)",
  ...
  log.split("\n").filter((l) => /POST \/api\/mcp 200/.test(l)).slice(-4).join("\n"),
  "",
