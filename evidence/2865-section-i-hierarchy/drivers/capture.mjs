...
for (const cell of PLAN.cells) {
  ...
  // A cell may need the app's OWN theme choice rather than the OS preference:
  // next-themes reads its stored value first, so `colorScheme` alone does not
  // flip a surface whose default is not "system".
  if (cell.initLocalStorage) {
    await ctx.addInitScript((kv) => {
      for (const [k, v] of Object.entries(kv)) {
        try { window.localStorage.setItem(k, v); } catch {}
      }
    }, cell.initLocalStorage);
  }
  ...
  // THE PICTURE IS TAKEN BEFORE THE COUNTS, deliberately. Some readings on this
  // surface are short-lived (a settled decision bar is replaced by the card's own
  // authoritative re-resolve within a second), and a record whose counts describe
  // a state the picture no longer shows is exactly the mislabel the contract
  // exists to refuse. Counting after the shot means the counts can only ever be
  // at-or-after what was photographed, never before it.
  ...
  const themeReadback = await page
    .evaluate(() => ({
      htmlClass: document.documentElement.className.split(/\s+/).filter((c) => c === "dark" || c === "light"),
