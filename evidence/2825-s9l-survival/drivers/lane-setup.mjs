...
console.log("sign-up", (await post("/api/auth/sign-up/email", { email: EMAIL, password: PASSWORD, name: process.env.LANE_NAME ?? "Lane Cap 2825" })).status);
...
const tpl = await client.query(
  `SELECT id, package_name FROM cinatra.agent_templates ORDER BY package_name LIMIT 30`,
);
const orgRows = await client.query(`SELECT id, name, slug FROM public.organization ORDER BY "createdAt"`);
await client.end();
...
console.log(JSON.stringify({ orgId, userId, orgs: orgRows.rows, templates: tpl.rows.map(r => r.package_name) }, null, 2));
